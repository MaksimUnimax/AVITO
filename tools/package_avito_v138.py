"""Create deterministic installable Avito Finder v1.0.38 ZIP and verify every round-tripped byte."""
import hashlib, io, json, subprocess, sys, zipfile
from pathlib import Path
root=Path(sys.argv[1]).resolve(); out=Path(sys.argv[2]).resolve()
manifest=json.loads((root/'manifest.json').read_text(encoding='utf-8'))
assert manifest['version']=='1.0.38', manifest['version']
...
with zipfile.ZipFile(b,'w',compression=zipfile.ZIP_DEFLATED,compresslevel=9) as z:
    for name,body in files.items():
        i=zipfile.ZipInfo('v138_work/'+name,date_time=(2026,9,13,0,0,0))
...
with zipfile.ZipFile(io.BytesIO(data)) as z:
    ...
    assert set(z.namelist())=={'v138_work/'+x for x in files}
    for name,body in files.items(): assert z.read('v138_work/'+name)==body,name
...
report={'version':'1.0.38','archive':out.name,'archive_sha256':hashlib.sha256(data).hexdigest(),'archive_bytes':len(data),'source_files':len(files),'source_hashes':{k:hashlib.sha256(v).hexdigest() for k,v in files.items()},'crc':'PASS','roundtrip_bytes':'PASS','runtime_syntax':'PASS','installed_live_acceptance':'NOT_RUN'}
(out.parent/'BUILD_v1.0.38.json').write_text(json.dumps(report,ensure_ascii=False,indent=2)+'\n',encoding='utf-8')
