# Avito Finder v1.0.35 — Assistant Form Terminal Gate

v1.0.35 is a minimal corrective release over the exact owner-supplied v1.0.34 source. It preserves the intentional rule that ordinary Markdown/code blocks are never executable commands, while removing the infinite wait when a completed assistant turn contains no Writing Block. The completed wrong-form turn now produces one same-chat format report and re-anchors for the next command. Validation-error wording is also explicit: the user's next message may be ordinary text, but the assistant's executable response must still be a Writing Block.

## Preserved safety boundary

- ordinary Markdown/code block => zero Avito commands;
- Writing Block remains the only executable assistant command surface;
- no Avito navigation or mutation occurs on the form error;
- one terminal same-chat report replaces endless `PROMPT_WAIT_WRITING_BLOCK`.
