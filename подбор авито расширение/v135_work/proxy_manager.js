/* Avito Finder v1.0.33 — Proxy.Market adapter for bounded unified connection recovery; egress probes are diagnostic, fresh Avito DOM is authoritative. */
"use strict";
(function(root, factory) {
  ...
  function validProxyHost(value) {
    ...
    if (host.includes(":")) {
      try { const ip = host.startsWith("[") ? host : `[${host}]`; return new URL(`http://${ip}/`).hostname.startsWith("["); } catch (_) { return false; }
    }
    if (/^[0-9.]+$/.test(host)) {
      const parts=host.split('.');return parts.length===4 && parts.every(p=>/^\d{1,3}$/.test(p)&&Number(p)<=255);
    }
    return host.replace(/\.$/,"").split('.').every(p=>p.length>0&&p.length<=63&&/^[A-Za-z0-9](?:[A-Za-z0-9-]*[A-Za-z0-9])?$/.test(p));
  }
  function credential(value, maximum) {
    const raw=String(value??"");
    if(raw.length>maximum || /[\r\n\x00]/.test(raw)) throw new Error('PROXY_CREDENTIAL_INVALID');
    return raw; // Credentials are opaque; never trim or silently truncate a password.
  }
  ...
  function normalizeProxyMarketRecord(record, context = {}) {
    ...
    const login = credential(record.login ?? record.username ?? record.user ?? nested(record, ["auth", "login"]), 256);
    const password = credential(record.password ?? record.pass ?? nested(record, ["auth", "password"]), 512);
    ...
    const rawHttpPort = record.http_port ?? record.httpPort ?? record.port ?? record.port_http ?? nested(record, ["ports", "http"]) ?? nested(record, ["http", "port"]) ?? record.port_from;
    let httpPort = int(rawHttpPort, null);
    if(rawHttpPort !== null && rawHttpPort !== undefined && !validPort(rawHttpPort)) return null;
  ...
  async function forceChangeIpByLink(requestFn, changeIpLink) {
    ...
    response?.discard?.(); // Only status is needed; close the bounded response body.
  ...
  async function fetchProxyMarketBalance(requestFn, apiKey) {
    ...
    let payload = null;
    try { payload = await response.json(); } catch (error) { if (response.ok) throw error; payload = null; }
  ...
  function apiCreateProxyBody(options = {}) {
    ...
    const rotation = options.rotation == null ? -1 : int(options.rotation, null);
    if (rotation === null || rotation < -1 || rotation > 60) throw new Error("PROXY_CREATE_ROTATION_INVALID");
  ...
  function makeManualTrafficProfile(input = {}) {
    ...
    const password = credential(input.password, 512);
  ...
  function normalizeEgressIp(value) {
    ...
    if(/^(?:\d{1,3}\.){3}\d{1,3}$/.test(raw)) {
      ...
      if(!a.every(n=>Number.isInteger(n)&&n>=0&&n<=255)) return '';
      if(a[0]===0 || a[0]===10 || a[0]===127 || a[0]>=224 || (a[0]===169&&a[1]===254) || (a[0]===172&&a[1]>=16&&a[1]<=31) || (a[0]===192&&a[1]===168) || (a[0]===100&&a[1]>=64&&a[1]<=127)) return '';
      return a.join('.');
    }
    if(!raw.includes(':') || /[^0-9a-f:]/i.test(raw)) return '';
    try {
      const canonical=new URL(`http://[${raw}]/`).hostname.slice(1,-1).toLowerCase();
      if(canonical==='::' || canonical==='::1' || /^(?:f[cd]|fe[89ab]|ff|::ffff:)/.test(canonical)) return '';
      return canonical;
    } catch(_) {return '';}
  }
  ...
  const PROXY_MUTATION_BUSY_STATUSES = new Set([
    ...
    "RECOVERING_AVITO_CONNECTION",
    "SEQUENTIAL_RECOVERY_WAIT",
    "SEQUENTIAL_RECOVERY_BLOCKED"
  ]);
