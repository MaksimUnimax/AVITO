...
class Harness:
 ...
 async def navigate(self,tid,url):
  ...
  await p.goto('about:blank');await p.set_content('<!doctype html><html><head><title>Offline fixture</title></head><body>'+html+'</body></html>');await p.evaluate('u=>window.AF_LOC=new URL(u)',url);await p.evaluate(SHIM,tid)
