...
async def main():
    async with async_playwright() as pw:
        with tempfile.TemporaryDirectory(prefix='avito-fixture-chromium-') as profile:
            ...
            async def load(html):
                ...
                for _ in range(50):
                    try:
                        ...
                        if ping.get('version')==EXPECTED_VERSION: return tid
            ...
            result={'environment':{'browser':ctx.browser.version,'extension_version':EXPECTED_VERSION,'scope':'real Chromium with installed production MV3 extension and local fulfilled fixtures only','live_avito_requests':0,'live_proxy_market_requests':0},'tests':RESULTS,'page_errors':errors,'request_count':len(requests),'passed':sum(t['status']=='PASS' for t in RESULTS),'failed':sum(t['status']=='FAIL' for t in RESULTS)}
