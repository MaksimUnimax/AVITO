/* Avito Finder v1.0.37 — unified connection recovery with explicit proxy telemetry, operation fencing and fresh public-document verification. */
"use strict";
importScripts("core.js", "proxy_manager.js", "recovery.js");
const Recovery = globalThis.AvitoFinderRecovery;
...
const RATE_LIMIT_RECOVERY_ALARM = "avito-finder-rate-limit-v128";
...
function cb(invoke, timeoutMs = 10000) { return Recovery.deadline(new Promise((resolve, reject) => invoke((value) => { const error = chrome.runtime.lastError; if (error) reject(new Error(error.message)); else resolve(value); })), timeoutMs, "CHROME_API_TIMEOUT"); }
...
const tabsSend = (tabId, message) => cb((done) => chrome.tabs.sendMessage(tabId, message, done), 60000);
...
const stateWriteLane = Recovery.serialLane();
const logWriteLane = Recovery.serialLane();
const proxyWriteLane = Recovery.serialLane();
async function saveState(next) { return stateWriteLane(async()=>{
  const raw=await storageGet(STATE_KEY), old=raw[STATE_KEY] || {};
  const newRun=next.status==='STARTING_BRIDGE_EXACT_CAPTURE' && next.user_started===true;
  if(!newRun && old.search_id && next.search_id && old.search_id!==next.search_id) return Core.makeState(old);
  if(!newRun && old.search_id===next.search_id && old.status==='CANCELLED_BY_USER' && next.status!=='CANCELLED_BY_USER') return Core.makeState(old);
  if(!next.allow_new_operation && old.operation_id && next.operation_id && old.operation_id!==next.operation_id && old.search_id===next.search_id) return Core.makeState(old);
  const state=Core.makeState({...next,updated_at:new Date().toISOString()}); delete state.allow_new_operation;
  await storageSet({[STATE_KEY]:state}); return state;
}); }
async function log(type, fields = {}) { return logWriteLane(async()=>{ const raw = await storageGet(LOG_KEY); const entries = Core.appendLog(raw[LOG_KEY], type, fields); await storageSet({ [LOG_KEY]: entries }); return entries.at(-1); }); }
...
function emptyProxyRuntime() {
  return {
    ...
    proxy_profile_epoch_at: null,
    ip_block_recovery_count: 0,
    last_ip_block_recovery: null,
    proxy_diagnostics: { last_auth_at: null, last_auth_profile_id: null, last_auth_host: null, last_auth_port: null, last_auth_attempt: null, last_auth_failure_at: null, last_auth_failure: null, last_proxy_error_at: null, last_proxy_error: null, last_avito_main_frame_at: null, last_avito_status_code: null, last_avito_tab_id: null, last_avito_request_id: null, last_avito_error_at: null, last_avito_error: null, last_egress_check_at: null, last_egress_ip: null, last_egress_error_at: null, last_egress_error: null },
    error: null
  };
}
...
async function saveProxyRuntime(next) { return proxyWriteLane(async()=>{
  const stored=await getProxyRuntime();
  const runtime={...emptyProxyRuntime(),...next,profiles:Array.isArray(next?.profiles)?next.profiles:[],package_candidates:Array.isArray(next?.package_candidates)?next.package_candidates:[],proxy_diagnostics:next?.proxy_diagnostics ? proxyDiagBase(next) : proxyDiagBase(stored)};
  await storageSet({[PROXY_RUNTIME_KEY]:runtime}); return runtime;
}); }
...
function proxyErrorText(error) { return Recovery.redactedError(error); }
function proxyDiagBase(runtime) {
  return { last_auth_at: null, last_auth_profile_id: null, last_auth_host: null, last_auth_port: null, last_auth_attempt: null, last_auth_failure_at: null, last_auth_failure: null, last_proxy_error_at: null, last_proxy_error: null, last_avito_main_frame_at: null, last_avito_status_code: null, last_avito_tab_id: null, last_avito_request_id: null, last_avito_error_at: null, last_avito_error: null, last_egress_check_at: null, last_egress_ip: null, last_egress_error_at: null, last_egress_error: null, ...(runtime?.proxy_diagnostics || {}) };
}
async function patchProxyDiagnostics(patch={}) { return proxyWriteLane(async()=>{
  const runtime=await getProxyRuntime();
  const next={...runtime,proxy_diagnostics:{...proxyDiagBase(runtime),...patch}};
  await storageSet({[PROXY_RUNTIME_KEY]:next}); return next;
}); }
const fixedProxyAvitoDomProbe = Core.publicPageInterruptionProbe;
async function probeVisibleAvitoInterruption(tab) {
  if (!tab?.id || !Core.isAvitoUrl(tab.url || "")) return { ip_block: false, rate_limit: false, captcha: false, href: String(tab?.url || ""), available: false };
  try {
    ...
    if (!data) return { ip_block: false, rate_limit: false, captcha: false, href: String(tab.url || ""), available: false };
    return { ...data, available: true };
  } catch (error) {
    return { ip_block: false, rate_limit: false, captcha: false, href: String(tab.url || ""), available: false, error: String(error?.message || error).slice(0, 220) };
  }
}
function normalizeFinderPac(data) {
  return String(data || "").replace(/^\/\/ avito-finder-proxy-epoch:[^\n]*\n/u, "");
}
function effectiveProxyMatchesActiveProfile(effective, profile) {
  if (!profile || effective?.levelOfControl !== "controlled_by_this_extension" || effective?.value?.mode !== "pac_script") return false;
  const expected = ProxyCore.buildAvitoOnlyPacConfig(profile);
  return normalizeFinderPac(effective?.value?.pacScript?.data) === normalizeFinderPac(expected?.pacScript?.data);
}
async function proxyDiagnosticView() {
  ...
  const exactPacMatch = effectiveProxyMatchesActiveProfile(effective, fullProfile);
  const profileEpochMs = Date.parse(runtime.proxy_profile_epoch_at || runtime.proxy_applied_at || "") || 0;
  ...
  const authProfileMatches = Boolean(fullProfile && (
    String(diag.last_auth_profile_id || "") === String(fullProfile.id) ||
    (String(diag.last_auth_host || "").toLowerCase() === String(fullProfile.host || "").toLowerCase() && Number(diag.last_auth_port) === Number(fullProfile.http_port))
  ));
  const authSeenForActiveProfile = Boolean(profileEpochMs && authAtMs >= profileEpochMs && authProfileMatches);
  let avitoTab = null;
  try {
  ...
  const proxyConfigured = runtime.mode === "proxy" && Boolean(summary) && exactPacMatch;
  ...
  else if (proxyConfigured && diag.last_proxy_error_at && (Date.parse(diag.last_proxy_error_at) || 0) >= profileEpochMs) status = "PROXY_ERROR";
  else if (proxyConfigured && avitoTab?.ip_block && avitoTab?.captcha) status = authSeenForActiveProfile ? "PROXY_AUTH_SEEN_AVITO_IP_BLOCK_CAPTCHA" : "PROXY_CONFIGURED_AVITO_IP_BLOCK_CAPTCHA_AUTH_NOT_OBSERVED";
  else if (proxyConfigured && avitoTab?.ip_block && authSeenForActiveProfile) status = "PROXY_AUTH_SEEN_AVITO_IP_BLOCK";
  else if (proxyConfigured && avitoTab?.ip_block) status = "PROXY_CONFIGURED_AVITO_IP_BLOCK_AUTH_NOT_OBSERVED";
  else if (proxyConfigured && avitoTab?.captcha) status = authSeenForActiveProfile ? "PROXY_AUTH_SEEN_AVITO_CAPTCHA" : "PROXY_CONFIGURED_AVITO_CAPTCHA_AUTH_NOT_OBSERVED";
  else if (proxyConfigured && avitoTab?.normal_avito && authSeenForActiveProfile && avitoNavigationAfterApply) status = "PROXY_AUTH_SEEN_AVITO_OK";
  ...
  return {
    ...
    configured_mode: effective?.value?.mode || null, effective_level: effective?.levelOfControl || null, exact_pac_match: exactPacMatch,
    proxy_applied_at: runtime.proxy_applied_at || null, proxy_profile_epoch_at: runtime.proxy_profile_epoch_at || null,
    auth_seen_for_active_profile: authSeenForActiveProfile, auth_seen_after_apply: authSeenForActiveProfile,
    last_auth_at: diag.last_auth_at, last_auth_profile_id: diag.last_auth_profile_id, last_auth_attempt: diag.last_auth_attempt, last_auth_failure: diag.last_auth_failure, last_auth_failure_at: diag.last_auth_failure_at,
    avito_navigation_after_apply: avitoNavigationAfterApply, last_avito_status_code: diag.last_avito_status_code,
    last_proxy_error: diag.last_proxy_error, last_proxy_error_at: diag.last_proxy_error_at,
    last_egress_check_at: diag.last_egress_check_at, last_egress_ip: diag.last_egress_ip, last_egress_error_at: diag.last_egress_error_at, last_egress_error: diag.last_egress_error,
    recovery: runtime.last_ip_block_recovery || null, key_present: Boolean(ProxyCore.text(secret.api_key, 512)), avito_tab: avitoTab
  };
}
...
async function collectProxyPackages(apiKey) {
  ...
  for (let page = 1; page <= 10; page += 1) {
    const result = await ProxyCore.fetchTrafficPackages(providerRequest, apiKey, { page, per_page: 10 });
...
async function checkProxyMarketKey(message = {}) {
  ...
  let balanceResult;
  try {
    balanceResult = await ProxyCore.fetchProxyMarketBalance(providerRequest, apiKey);
  } catch (error) {
    const code = proxyErrorText(error);
...
async function collectProxyPagesDetailed(apiKey, options = {}) {
  ...
  for (let page = 1; page <= 50 && collected.length < ProxyCore.MAX_PROFILES; page += 1) {
    ...
    const payload = await ProxyCore.fetchProxyMarketPage(providerRequest, apiKey, requestOptions);
...
async function syncProxyMarket(message = {}) {
  ...
  try {
    const balanceResult = await ProxyCore.fetchProxyMarketBalance(providerRequest, apiKey);
    let packageCandidates = [];
    let activePackages = [];
    let packageWarning = null;
    try {
    ...
    if (!collected.length) {
      ...
      await saveProxySecret({ ...priorSecret, api_key: apiKey, package_id: packageId, order_id: orderId });
      const next = await saveProxyRuntime({
        ...priorRuntime,
        profiles: priorRuntime.profiles,
        selected_profile_id: priorRuntime.selected_profile_id,
    ...
    // Sync is catalogue discovery, not a hidden live-profile switch. Retain
    // the applied profile credentials until an explicit checked apply.
    const activeCached=priorRuntime.mode==='proxy'?(priorSecret.profiles||[]).find(p=>String(p.id)===String(priorRuntime.selected_profile_id)):null;
    const discovered=activeCached?[activeCached,...collected.filter(p=>String(p.id)!==String(activeCached.id))]:collected;
    const canonicalProfiles = await savePersistedProxyProfiles(ProxyCore.dedupeProfileSet(discovered));
    const hiddenIds = await getHiddenProxyProfileIds();
    const visibleProfiles = canonicalProfiles.filter((profile) => !hiddenIds.has(String(profile.id)) || String(profile.id)===String(activeCached?.id));
...
async function saveManualTrafficProfile(message = {}) {
  ...
  try {
    ...
    if (priorRuntime.mode==='proxy' && String(priorRuntime.selected_profile_id)===String(profile.id)) throw new Error('PROXY_ACTIVE_PROFILE_EDIT_REQUIRES_MANUAL_DIRECT');
    ...
    const next = await saveProxyRuntime({ ...priorRuntime, profiles: summaries, selected_profile_id: priorRuntime.mode==='proxy'?priorRuntime.selected_profile_id:profile.id, package_id: profile.package_id || priorRuntime.package_id || null, error: null, last_operation: "PROXY_MANUAL_TRAFFIC_PROFILE_SAVED" });
...
async function deleteProxyProfileLocally(profileId) {
  ...
  const runtime = await getProxyRuntime();
  const secret = await getProxySecret();
  const removingActive = runtime.mode === "proxy" && String(runtime.selected_profile_id || "") === id;
  if (removingActive) {const switched = await setProxyDirect();if(!switched.ok)return switched;}
  ...
  const selected = summaries.some((profile) => String(profile.id) === String(runtime.selected_profile_id || "")) ? runtime.selected_profile_id : (summaries[0]?.id || null);
  const currentRuntime=await getProxyRuntime();
  await saveProxySecret({ ...secret, profiles: visible, active_profile_id: removingActive ? null : secret.active_profile_id });
  const next = await saveProxyRuntime({ ...currentRuntime, profiles: summaries, selected_profile_id: selected, error: null, last_operation: "PROXY_PROFILE_REMOVED_LOCAL" });
  await log("PROXY_PROFILE_REMOVED_LOCAL", { profile_id: id, remaining_profile_count: summaries.length });
  return { ok: true, proxy: { ...next, active_profile: next.mode==='proxy'?summaries.find(p=>String(p.id)===String(next.selected_profile_id))||null:null }, note: "REMOTE_ENDPOINT_UNCHANGED" };
}
...
function trustedExtensionUiSender(sender) {
  return sender?.id === chrome.runtime.id && String(sender?.url || "") === chrome.runtime.getURL("popup.html");
}
function trustedContentSender(sender, hostKind) {
  if(sender?.id !== chrome.runtime.id || !Number.isInteger(sender?.tab?.id) || (sender.frameId != null && sender.frameId !== 0)) return false;
  const url=String(sender.url || sender.tab.url || "");
  const allowed = hostKind==='chat' ? Core.isChatGPTUrl(url) : Core.isPublicAvitoUrl(url);
  if(!allowed) return false;
  try { return !sender.origin || sender.origin===new URL(url).origin; } catch (_) { return false; }
}
async function releaseFinderProxySetting() {
  await cb(done=>chrome.proxy.settings.clear({scope:'regular'},done));
  return proxySettingsGet(); // Clear only this extension's setting; system/other extensions regain precedence.
}
...
async function createProxyMarketEndpoint(message = {}) {
  ...
  const rotation = message?.rotation == null ? -1 : ProxyCore.int(message.rotation, null);
  if(message?.rotation != null && rotation===null) return recordProxyFailure("PROXY_MARKET_ENDPOINT_CREATE","PROXY_CREATE_ROTATION_INVALID");
  let intent=null;
  try {
    const body=ProxyCore.apiCreateProxyBody({package_id:packageId,country,rotation});
    const old=(await storageGet('af_proxy_manual_create_v129')).af_proxy_manual_create_v129;
    if(old?.state==='DISPATCHED_OUTCOME_UNKNOWN') return {ok:false,error:'PROXY_CREATE_OUTCOME_UNKNOWN_RECONCILE_REQUIRED'};
    intent={state:'DISPATCHED_OUTCOME_UNKNOWN',requested_at:Date.now(),package_id:body.packageId,country:body.country,rotation:body.rotation};
    await storageSet({af_proxy_manual_create_v129:intent});
    const createResult = await ProxyCore.createProxyInPackage(providerRequest, apiKey, { package_id: packageId, country, rotation });
    await storageSet({af_proxy_manual_create_v129:{...intent,state:'ACKNOWLEDGED',acknowledged_at:Date.now()}});
    ...
    const code = proxyErrorText(error);
    if(intent && [400,401,403,404,422].includes(error?.status)) await storageSet({af_proxy_manual_create_v129:{...intent,state:'REJECTED',http_status:error.status}});
    const next = await saveProxyRuntime({ ...await getProxyRuntime(), error: code, last_operation: "PROXY_MARKET_ENDPOINT_CREATE_FAILED" });
...
async function refreshProxyAuthHandlerBehavior() {
  // Listener is registered once. handlerBehaviorChanged is NOT an auth-cache flush.
  return;
}
async function applyProxyProfileInternal(profile, operation = "PROXY_PROFILE_APPLIED_AUTO", extra = {}, options = {}) {
  ...
  const secret = await getProxySecret();
  const priorRuntime = await getProxyRuntime();
  await storageSet({af_proxy_apply_intent_v125:{previous_profile_id:priorRuntime.selected_profile_id,previous_mode:priorRuntime.mode,requested_profile_id:profile.id,at:Date.now()}});
  await saveProxySecret({...secret,pending_auth_profile_id:profile.id,profiles:ProxyCore.dedupeProfileSet([profile,...(secret.profiles||[])])});
  try { await proxySettingsSet(config); await assertEffectiveProxy(config); }
  catch(error) {
    // Roll back only settings still owned by us, not an administrator/another extension.
    const effective = await proxySettingsGet().catch(()=>null);
    const priorProfile = (secret.profiles || []).find(p=>String(p.id)===String(priorRuntime.selected_profile_id));
    if (effective?.levelOfControl === "controlled_by_this_extension") {
      const priorConfig = priorRuntime.mode === "proxy" && priorProfile ? ProxyCore.buildAvitoOnlyPacConfig(priorProfile) : (priorRuntime.mode === "direct" ? ProxyCore.directConfig() : null);
      if (priorRuntime.mode==='direct') await releaseFinderProxySetting();
      else if (priorConfig) { await proxySettingsSet(priorConfig); await assertEffectiveProxy(priorConfig); }
    }
    await saveProxySecret({...secret,pending_auth_profile_id:null});
    await storageRemove('af_proxy_apply_intent_v125');
    throw error;
  }
  await saveProxySecret({...await getProxySecret(),active_profile_id:profile.id,pending_auth_profile_id:null});
  proxyAuthAttempts.clear();
  await refreshProxyAuthHandlerBehavior();
  const runtime = await getProxyRuntime();
  const dataSaverActive = runtime.data_saver_enabled !== false;
  let dataSaverError = null;
  try { await syncProxyDataSaverRules(dataSaverActive); } catch(error) { dataSaverError = proxyErrorText(error); }
  const appliedAt = new Date().toISOString();
  const sameProfileEpoch = runtime.mode === "proxy" && String(runtime.selected_profile_id || "") === String(profile.id) && runtime.proxy_profile_epoch_at;
  const profileEpochAt = sameProfileEpoch ? runtime.proxy_profile_epoch_at : appliedAt;
  const next = await saveProxyRuntime({
    ...runtime,
    mode: "proxy",
    selected_profile_id: profile.id,
    proxy_applied_at: appliedAt,
    proxy_profile_epoch_at: profileEpochAt,
    proxy_diagnostics: options.preserve_proxy_diagnostics === true ? proxyDiagBase(runtime) : proxyDiagBase(null),
    data_saver_active: dataSaverError ? false : dataSaverActive,
    error: dataSaverError,
...
// v1.0.25: one transaction for every interruption source. Probe IP is telemetry,
// never a claim about the IP used by Avito's separate HTTPS connection.
const recoveryFlight = Recovery.singleFlight();
const proxyControlLane = Recovery.serialLane();
const recoveryControllers = new Map();
const runtimeWakeFlight = Recovery.singleFlight();
let workerRecoveryOwner = null;
let proxyUiOwner = false;
let proxyRequestSignal = null;
const captureLane = Recovery.serialLane();
const startFlight = Recovery.singleFlight();
async function assertCurrentOperation(expected) {
  const current = await getState();
  if (!Recovery.sameOperation(expected,current) || current.status === 'CANCELLED_BY_USER') throw new Error('OPERATION_CANCELLED');
  return current;
}
function providerRequest(url, options) { return Recovery.fetchBounded(globalThis.fetch,url,options,{timeout_ms:12000,signal:proxyRequestSignal}); }
async function assertEffectiveProxy(config) {
  const effective = await proxySettingsGet();
  if (!Recovery.pacMatches(effective,config)) throw new Error('PROXY_CONFIGURATION_NOT_EFFECTIVE');
  return effective;
}
async function sampleRecoveryEgress(profile, record, signal) {
  const observedAt = new Date().toISOString();
  try {
    const response=await Recovery.fetchBounded(globalThis.fetch,ProxyCore.egressCheckUrl(`${record.attempt}-${Date.now()}`,'api.ipify.org'),{
      method:'GET',headers:{Accept:'application/json'},cache:'no-store',credentials:'omit',redirect:'error'
    },{timeout_ms:5000,max_chars:4096,signal});
    if(!response.ok) throw new Error(`PROXY_EGRESS_CHECK_HTTP_${response.status}`);
    const ip=ProxyCore.parseEgressIpPayload(await response.json());
    if(!ip || ip.includes(':')) throw new Error('PROXY_EGRESS_IPV4_REQUIRED');
    await patchProxyDiagnostics({last_egress_check_at:observedAt,last_egress_ip:ip,last_egress_error_at:null,last_egress_error:null});
    await log('PROXY_EGRESS_OBSERVED',{attempt:record.attempt,profile_id:profile.id,host:'api.ipify.org',ip,scope:'probe_only_not_avito'});
    return {ip,host:'api.ipify.org',family:4,observed_at:observedAt,error:null};
  } catch(error) {
    if(signal.aborted) throw new Error('OPERATION_CANCELLED');
    const code=Recovery.redactedError(error);
    await patchProxyDiagnostics({last_egress_check_at:observedAt,last_egress_ip:null,last_egress_error_at:observedAt,last_egress_error:code});
    await log('PROXY_EGRESS_DIAGNOSTIC_UNAVAILABLE',{attempt:record.attempt,error:code});
    return {ip:null,host:'api.ipify.org',family:4,observed_at:observedAt,error:code};
  }
}
async function reserveRecoveryEndpoint(state, record) {
  state=await assertCurrentOperation(state);
  const current=state.connection_recovery;
  if(current?.create) return null;
  const updated={...record,create:{state:'DISPATCHED_OUTCOME_UNKNOWN',requested_at:Date.now()}};
  await saveState({...state,connection_recovery:updated});
  return updated;
}
async function maybeCreateRecoveryEndpoint(state, record, profile, signal) {
  const secret=await getProxySecret();
  const apiKey=ProxyCore.text(secret.api_key,512);
  const packageId=ProxyCore.int(profile.package_id || secret.package_id,null);
  if(record.attempt < 3) return profile;
  if(!apiKey || !packageId) {
    const reason=!apiKey?'API_KEY_UNAVAILABLE':'PACKAGE_ID_UNAVAILABLE';
    state=await assertCurrentOperation(state);
    const create={state:'SKIPPED',reason,attempt:record.attempt,at:Date.now()};
    await saveState({...state,connection_recovery:{...state.connection_recovery,create}});
    await log('PROXY_RECOVERY_ENDPOINT_SKIPPED',{attempt:record.attempt,reason,no_blind_fallback:true});
    return profile;
  }
  const reserved=await reserveRecoveryEndpoint(state,record);
  if(!reserved) return profile;
  const beforeIds=new Set((secret.profiles||[]).map(p=>String(p.id)));
  const request=(url,options)=>Recovery.fetchBounded(globalThis.fetch,url,options,{timeout_ms:10000,signal});
  try {
    await ProxyCore.createProxyInPackage(request,apiKey,{package_id:packageId,country:profile.country||'ru',rotation:-1});
    state=await assertCurrentOperation(state);
    await saveState({...state,connection_recovery:{...state.connection_recovery,create:{...reserved.create,state:'ACKNOWLEDGED'}}});
    // One documented list read. An ambiguous create is never repeated.
    const payload=await ProxyCore.fetchProxyMarketPage(request,apiKey,{package_id:packageId,type:'all',page:1,page_size:50,sort:1});
    const profiles=ProxyCore.normalizeProxyMarketList(payload,{package_id:packageId});
    const candidate=profiles.find(p=>!beforeIds.has(String(p.id)) && p.rotation_settings?.rotate===-1 && p.package_id===packageId);
    if(!candidate) throw new Error('PROXY_CREATED_ENDPOINT_RECONCILIATION_REQUIRED');
    state=await assertCurrentOperation(state);
    const all=ProxyCore.dedupeProfileSet([candidate,...(secret.profiles||[])]);
    await savePersistedProxyProfiles(all);
    await saveProxySecret({...secret,profiles:all});
    await saveProxyRuntime({...await getProxyRuntime(),profiles:all.map(ProxyCore.profileSummary)});
    await saveState({...state,connection_recovery:{...state.connection_recovery,create:{...reserved.create,state:'IDENTIFIED',profile_id:candidate.id}}});
    await log('PROXY_RECOVERY_ENDPOINT_IDENTIFIED',{profile_id:candidate.id,attempt:record.attempt,package_id:packageId});
    return candidate;
  } catch(error) {
    if(signal.aborted) throw new Error('OPERATION_CANCELLED');
    await log('PROXY_RECOVERY_CREATE_RECONCILE_REQUIRED',{error:Recovery.redactedError(error),attempt:record.attempt,no_blind_retry:true});
    return profile;
  }
}
async function prepareRecoveryProxy(state, record, signal) {
  const runtime=await getProxyRuntime(), secret=await getProxySecret();
  if(runtime.mode!=='proxy') throw new Error('AVITO_IP_BLOCK_PROXY_NOT_ACTIVE');
  let profile=(secret.profiles||[]).find(p=>String(p.id)===String(runtime.selected_profile_id));
  if(!profile) throw new Error('PROXY_PROFILE_CREDENTIALS_UNAVAILABLE');
  await assertCurrentOperation(state);
  // Persist BEFORE changing settings. Startup recovery can always restore the
  // last selected working profile after a worker/browser interruption.
  await storageSet({af_proxy_transaction_v125:{profile_id:profile.id,operation_id:Recovery.operationKey(state),started_at:Date.now()}});
  let result;
  try {
    await applyProxyProfileInternal(profile,'PROXY_RECOVERY_DIAGNOSTIC_APPLIED',{}, {extra_hosts:['api.ipify.org'],pac_nonce:`before-${record.attempt}-${Date.now()}`,preserve_proxy_diagnostics:true});
    const before=await sampleRecoveryEgress(profile,record,signal);
    await assertCurrentOperation(state);
    if(record.attempt>=2 && profile.rotation_settings?.rotate_can_change===true && profile.rotation_settings?.change_ip_link) {
      let latest=await assertCurrentOperation(state);
      const priorRotation=latest.connection_recovery?.provider_rotation || null;
      if(priorRotation?.attempt===record.attempt && ['DISPATCHED_OUTCOME_UNKNOWN','ACKNOWLEDGED'].includes(priorRotation.state)) {
        await log('PROXY_PROVIDER_ROTATION_NOT_REPEATED_AFTER_RESTART',{attempt:record.attempt,profile_id:profile.id,state:priorRotation.state});
      } else {
        const reservation={attempt:record.attempt,state:'DISPATCHED_OUTCOME_UNKNOWN',requested_at:Date.now(),profile_id:profile.id};
        latest=await saveState({...latest,connection_recovery:{...latest.connection_recovery,provider_rotation:reservation}});
        try {
          const request=(url,options)=>Recovery.fetchBounded(globalThis.fetch,url,options,{timeout_ms:8000,signal});
          await ProxyCore.forceChangeIpByLink(request,profile.rotation_settings.change_ip_link);
          latest=await assertCurrentOperation(latest);
          await saveState({...latest,connection_recovery:{...latest.connection_recovery,provider_rotation:{...reservation,state:'ACKNOWLEDGED',acknowledged_at:Date.now()}}});
          await log('PROXY_PROVIDER_ROTATION_ACKNOWLEDGED',{attempt:record.attempt,profile_id:profile.id,ip_change_proven:false});
        } catch(error) {if(signal.aborted) throw error; await log('PROXY_PROVIDER_ROTATION_FAILED',{error:Recovery.redactedError(error),attempt:record.attempt,not_retried_blindly:true});}
      }
    }
    profile=await maybeCreateRecoveryEndpoint(state,(await getState()).connection_recovery,profile,signal);
    await assertCurrentOperation(state);
    // Chrome owns CONNECT pooling. A PAC nonce requests reconfiguration; it
    // does NOT assert that every connection or Avito exit IP was replaced.
    await applyProxyProfileInternal(profile,'PROXY_RECOVERY_RECONFIGURED',{}, {extra_hosts:['api.ipify.org'],pac_nonce:`after-${record.attempt}-${Date.now()}`,preserve_proxy_diagnostics:true});
    const after=await sampleRecoveryEgress(profile,record,signal);
    const latestRecovery=(await getState()).connection_recovery || {};
    const transportStatus=before.ip && after.ip ? 'PROBES_COMPLETE' : (before.ip || after.ip ? 'PROBES_PARTIAL' : 'PROBES_UNAVAILABLE');
    result={ok:true,strategy:'verified_config_then_fresh_avito_document',profile_id:profile.id,proxy_config_verified:true,transport_status:transportStatus,probe_before:before.ip,probe_after:after.ip,probe_before_error:before.error||null,probe_after_error:after.error||null,probe_before_at:before.observed_at||null,probe_after_at:after.observed_at||null,probe_ip_changed:Boolean(before.ip&&after.ip&&before.ip!==after.ip),endpoint_create:latestRecovery.create||null,avito_exit_ip_verified:false};
  } finally {
    // No blackhole endpoint and no diagnostic hosts may remain installed.
    // No DIRECT fallback is used for Avito, even after an exception/cancel.
    await applyProxyProfileInternal(profile,'PROXY_RECOVERY_ROUTE_RESTORED',{}, {pac_nonce:`active-${Date.now()}`,preserve_proxy_diagnostics:true});
    await storageRemove('af_proxy_transaction_v125');
  }
  await assertCurrentOperation(state);
  return result;
}
async function restoreInterruptedProxyTransaction() {
  const raw=await storageGet(['af_proxy_transaction_v125','af_proxy_apply_intent_v125']);
  if(!raw.af_proxy_transaction_v125 && !raw.af_proxy_apply_intent_v125) return;
  const runtime=await getProxyRuntime(), secret=await getProxySecret();
  const profile=(secret.profiles||[]).find(p=>String(p.id)===String(runtime.selected_profile_id || raw.af_proxy_apply_intent_v125?.previous_profile_id || raw.af_proxy_transaction_v125?.profile_id));
  if(runtime.mode==='proxy' && profile && raw.af_proxy_apply_intent_v125?.requested_mode!=='release') await applyProxyProfileInternal(profile,'PROXY_INTERRUPTED_ROUTE_RESTORED');
  else if(runtime.mode==='direct' || raw.af_proxy_apply_intent_v125?.requested_mode==='release') {
    await releaseFinderProxySetting();
    await saveProxySecret({...secret,pending_auth_profile_id:null,active_profile_id:null});
    await saveProxyRuntime({...runtime,mode:'direct',data_saver_active:false});
    await syncProxyDataSaverRules(false);
  }
  else throw new Error('PROXY_INTERRUPTED_ROUTE_RESTORE_REQUIRES_PROFILE');
  await storageRemove(['af_proxy_transaction_v125','af_proxy_apply_intent_v125']);
}
function armRuntimeWatchdog() { return scheduleOwnedAlarm("avito-finder-runtime-v125",Date.now()+30000); }
async function recoverRuntimeOnWake() {
  return runtimeWakeFlight('wake',async()=>{
    if(workerRecoveryOwner || proxyUiOwner || avitoCompletionLocks.size || reportDeliveryOwners.size) {armRuntimeWatchdog();return;}
    await proxyControlLane(()=>restoreInterruptedProxyTransaction());
    let state=await getState();
    if(state.status==='CANCELLED_BY_USER' || !state.search_id) return;
    // v1.0.32 migration: a CAPTCHA failure could already have been reported and
    // then followed by one or more validator reports before the manual gate had
    // an explicit state field. Re-establish that gate read-only from the still
    // visible Avito tab; never navigate, reload or read user message text here.
    if (state.status === 'WAITING_FOR_NEXT_ASSISTANT_FORM' && !state.manual_gate_kind && Number.isInteger(state.avito_tab_id)) {
      const captchaTab = await getTab(state.avito_tab_id);
      if (captchaTab && captchaTab.windowId === state.current_window_id && Core.isAvitoUrl(captchaTab.url || '')) {
        try {
          const interruption = await probeVisibleAvitoInterruption(captchaTab);
          if (interruption?.captcha === true) {
            state = await saveState({ ...state, phase: 'CAPTCHA_MANUAL_WAIT', manual_gate_kind: 'CAPTCHA', blocked_reason: null });
            await log('CAPTCHA_MANUAL_GATE_MIGRATED_FROM_VISIBLE_PAGE', { search_id: state.search_id, tab_id: captchaTab.id, user_text_read: false });
          }
        } catch (error) {
          await log('CAPTCHA_MANUAL_GATE_MIGRATION_PROBE_FAILED', { search_id: state.search_id, tab_id: captchaTab.id, reason: Recovery.redactedError(error) });
        }
      }
    }
    if (state.status === 'START_DELIVERY_UNCERTAIN' && state.start_send_receipt && state.start_send_attempted !== false) {
      if (state.start_ack_budget && (state.start_ack_budget.attempt >= START_ACK_MAX_RECHECKS || Date.now() >= state.start_ack_budget.deadline_at)) return;
      return reconcileUncertainStart(state);
    }
    if (['REPORT_DELIVERY_BLOCKED','REPORT_DELIVERY_IN_PROGRESS','REPORT_DELIVERY_UNCERTAIN','REPORT_READY_IN_COMPOSER'].includes(state.status) && state.report) {
      if (state.report_ack_budget && (state.report_ack_budget.attempt >= REPORT_ACK_MAX_RECHECKS || Date.now() >= state.report_ack_budget.deadline_at)) return;
      // Read-only reconciliation is safe even when Finder never clicked Send.
      // This is required for the supported manual-send recovery path: a staged
      // report may be sent by the operator after ChatGPT temporarily hides or
      // replaces its Send control. Reconciliation must discover that exact user
      // turn and advance to the next Writing Block without a second Send.
      return reconcileBlockedReport(state);
    }
    if (['REPORT_READY_FOR_DELIVERY','AVITO_FAILURE_REPORT_READY','VALIDATION_ERROR_REPORT_READY'].includes(state.status) && state.report) return deliverReportToPinnedChat(state,state.report,state.report_kind || 'avito_failure');
    if (['RATE_LIMIT_BACKOFF','RECOVERING_AVITO_CONNECTION'].includes(state.status) && state.connection_recovery) {
      return resumePersistedConnectionRecovery(state);
    }
    if (state.status === "SEQUENTIAL_GAP_WAIT" && Number(state.sequential_review?.resume_after || 0) > Date.now()) { scheduleSequentialGap(state.sequential_review.resume_after); return; }
    if (['AVITO_TAB_ACTIVE','SEQUENTIAL_RECOVERY_WAIT','SEQUENTIAL_GAP_WAIT'].includes(state.status)) {
      const record=state.connection_recovery;
      if(state.command_mode==='AVITO_UI' && Recovery.replayPolicy(state.ui_action_plan)==='NEVER') return blockAvitoPageReady(state,'AVITO_WORKER_INTERRUPTED_MUTATING_PLAN_NOT_REPLAYED');
      // The old response channel may have closed while the DOM adapter remained alive.
      const contentTabs=[state.avito_tab_id,state.sequential_review?.in_flight?.child_tab_id].filter(Number.isInteger);
      for(const tabId of new Set(contentTabs)) await tabsSend(tabId,{type:'AF_CANCEL_AVITO_OPERATION'}).catch(()=>{});
      if(Recovery.replayPolicy(state.ui_action_plan)==='CHECKPOINT_ONLY' && state.sequential_review?.candidates?.length) {
        const plan={...state.ui_action_plan,steps:[{type:'RESUME_EXPLICIT_LISTING_QUEUE'}]};
        state=await saveState({...state,ui_action_plan:plan});
      }
      const resumed=await saveState({...state,status:'WAITING_FOR_AVITO_PAGE_READY',phase:'AVITO_WORKER_OPERATION_RECONCILED',avito_reload_expected:false,avito_navigation_expected:false,avito_ready_deadline_at:new Date(Date.now()+AVITO_READY_TIMEOUT_MS).toISOString()});
      await log('AVITO_RECOVERY_RESUMED_AFTER_WORKER_RESTART',{search_id:state.search_id,attempt:record?.attempt || 0,mutation_replayed:false});
      scheduleAvitoReadyDeadline(resumed,50);
    } else if(state.status==='WAITING_FOR_AVITO_PAGE_READY') await reconcilePendingAvitoReadiness();
    else if(state.status==='WAITING_FOR_UI_CLICK_RECONCILIATION') await reconcilePendingUiClickHandoff();
    else if(state.status==='COMMAND_CAPTURED') await queueAvitoTask(state,{mode:state.command_mode,ui_plan:state.ui_action_plan,diagnostic:state.diagnostic_request});
    else if(state.status==='FORM_TEXT_CAPTURED_UNVALIDATED') {
      // Migration from <=1.0.33. No irreversible Avito action can have occurred
      // before this state; forget the provisional consumption and let the same
      // Writing Block be captured again after deterministic validation.
      const waiting = await saveState({
        ...state,
        status: 'WAITING_FOR_NEXT_ASSISTANT_FORM',
        phase: 'LOCAL_VALIDATION_INTERRUPTION_RECOVERED',
        operation_id: null,
        last_consumed_form_key: null,
        assistant_turn_id: null,
        blocked_reason: null
      });
      await log('FORM_TEXT_CAPTURED_UNVALIDATED_RECOVERED_READ_ONLY', { search_id: waiting.search_id, anchor_turn_id: waiting.anchor_turn_id });
      return;
    }
  });
}

async function proxyUiRequest(message) {
  const operations = {
    AF_PROXY_KEY_CHECK: () => checkProxyMarketKey(message), AF_PROXY_KEY_IMPORT: () => importProxyKeyBundle(message),
    AF_PROXY_MARKET_SYNC: () => syncProxyMarket(message), AF_PROXY_ENDPOINT_CREATE: () => createProxyMarketEndpoint(message),
    AF_PROXY_MANUAL_PROFILE_SAVE: () => saveManualTrafficProfile(message), AF_PROXY_APPLY: () => applyProxyProfile(message.profile_id),
    AF_PROXY_DIRECT: () => setProxyDirect(), AF_PROXY_CREATE_ACKNOWLEDGE: async()=>{if(message.confirmed !== true) return {ok:false,error:'MANUAL_RECONCILIATION_CONFIRMATION_REQUIRED'}; const old=(await storageGet('af_proxy_manual_create_v129')).af_proxy_manual_create_v129; if(old?.state!=='DISPATCHED_OUTCOME_UNKNOWN')return {ok:false,error:'NO_UNCERTAIN_CREATE'}; await storageSet({af_proxy_manual_create_v129:{...old,state:'OPERATOR_RECONCILED',reconciled_at:Date.now()}}); return {ok:true};}, AF_PROXY_DATA_SAVER: () => setProxyDataSaver(message.enabled === true),
    AF_PROXY_TRAFFIC_REFRESH: () => refreshProxyTrafficUsage(), AF_PROXY_PROFILE_DELETE: () => deleteProxyProfileLocally(message.profile_id),
    AF_PROXY_PROFILE_RESTORE: () => restoreHiddenProxyProfiles()
  };
  const operation = operations[message.type];
  if (!operation) return {ok:false,error:'UNKNOWN_PROXY_MESSAGE'};
  if (proxyUiOwner || workerRecoveryOwner) return {ok:false,error:'PROXY_MUTATION_WAIT_FOR_CURRENT_STEP'};
  return proxyControlLane(async()=>{
    const busy = proxyMutationError(await getState());
    if (busy) return {ok:false,error:busy};
    proxyUiOwner = true;
    const controller = new AbortController(); proxyRequestSignal = controller.signal;
    const timeout = setTimeout(()=>controller.abort(),60000);
    try { return await operation(); }
    finally {clearTimeout(timeout);proxyRequestSignal=null;proxyUiOwner=false;await restoreInterruptedProxyTransaction();}
  });
}
...
async function setProxyDirect() {
  ...
  try {
    await storageSet({af_proxy_apply_intent_v125:{previous_mode:'direct',requested_mode:'release',at:Date.now()}});
    await releaseFinderProxySetting();
    await syncProxyDataSaverRules(false);
    const secret = await getProxySecret();
    ...
    await refreshProxyAuthHandlerBehavior();
    const runtime = await getProxyRuntime();
    const next = await saveProxyRuntime({ ...runtime, mode: "direct", proxy_applied_at: null, proxy_profile_epoch_at: null, proxy_diagnostics: proxyDiagBase(null), data_saver_active: false, error: null, last_operation: "PROXY_DIRECT_APPLIED_MANUAL" });
    const effective = await proxySettingsGet();
    await storageRemove('af_proxy_apply_intent_v125');
...
function avitoReadyProbeAccepted(response, tab, windowId, state = null) {
  ...
  if (state?.avito_reload_expected === true) {
    ...
    if(!Number.isFinite(currentTimeOrigin) || currentTimeOrigin<=0) return false;
    if(previousTimeOrigin>0 && currentTimeOrigin===previousTimeOrigin) return false;
    if(!(previousTimeOrigin>0) && !(currentTimeOrigin>=Number(state.avito_reload_requested_at||Infinity))) return false;
  }
  return true;
}
...
async function blockAvitoPageReady(state, reason, details = {}) {
  ...
  if(!Recovery.sameOperation(state,latest) || ['CANCELLED_BY_USER','REPORT_SENT_CONFIRMED','WAITING_FOR_NEXT_ASSISTANT_FORM','REPORT_DELIVERY_IN_PROGRESS'].includes(latest.status)) return latest;
  ...
  return returnAvitoRuntimeFailureToPinnedChat(next,reason);
}
...
async function selectVisibleAvitoTab(windowId, requestedUrl = "") {
...
function directPublicAvitoUrl(value) {
  try {
    const url = new URL(String(value || ""));
    if (!Core.isPublicAvitoUrl(url.href) || url.protocol !== "https:" || !/^(?:www\.)?avito\.ru$/iu.test(url.hostname) || url.username || url.password) return null;
    url.hash = "";
    return url.href;
  } catch (_) { return null; }
}
...
async function applyRequestedVisibleAvitoUrl(state, tab, requestedUrl, purpose, source) {
  ...
  if(state.command_mode==='DIAGNOSE_DOM' && state.diagnostic_request?.allow_navigation===false) {
    if(url && !sameVisibleAvitoRoute(url,tab.url)) throw new Error('DIAGNOSTIC_READ_ONLY_URL_MISMATCH');
    return {tab,navigated:false,reload_requested:false,source,previous_url:tab.url,requested_url:url};
  }
  ...
  state = await assertCurrentOperation(state);
  const intent = { operation_id: Recovery.operationKey(state), tab_id: tab.id,
    requested_url: url, from_url: currentUrl, created_at: Date.now() };
  await saveState({ ...state, command_navigation_intent: intent });
  await log("AVITO_COMMAND_NAVIGATION_INTENT", { search_id: state.search_id, tab_id: tab.id, requested_url: url });
  await assertCurrentOperation(state);
...
async function ensureAvitoTarget(state, requestedUrl, purpose, _policy = {}) {
  ...
  try {
    const selected = await selectVisibleAvitoTab(state.current_window_id, requestedUrl);
...
async function visibleAvitoDelay(state, stage, ms) {
  ...
  const latest = await getState();
  if (!Recovery.sameOperation(state,latest) || latest.status === "CANCELLED_BY_USER") throw new Error("AVITO_VISIBLE_DELAY_CANCELLED");
...
async function requireReadyAvitoTarget(tabId, windowId, stateOverride = null) {
  const target = await getTab(tabId);
  if (!Core.isPublicAvitoUrl(target?.url || "")) throw new Error("PUBLIC_AVITO_PAGE_REQUIRED");
...
async function sendAvito(tabId, windowId, message) {
  const owner = await getState();
  await assertCurrentOperation(owner);
  if (owner.status === "CANCELLED_BY_USER") throw new Error("OPERATION_CANCELLED");
  const ready = await requireReadyAvitoTarget(tabId, windowId);
  let ping = null;
  try { ping = await tabsSend(tabId, {type:"AF_AVITO_PING"}); } catch (_) {}
  if (!ping?.ok || ping.version !== chrome.runtime.getManifest().version) {
    await execute({target:{tabId},files:["core.js","avito_content.js"],injectImmediately:true});
    ping = await tabsSend(tabId,{type:"AF_AVITO_PING"});
    if (!ping?.ok || ping.version !== chrome.runtime.getManifest().version) throw new Error("AVITO_ADAPTER_VERSION_MISMATCH_RELOAD_REQUIRED");
    await log("CONTENT_SCRIPT_INJECTED",{role:"avito",tab:shortTab(ready),files:["core.js","avito_content.js"]});
  }
  await assertCurrentOperation(owner);
  // The command is sent ONCE. A closed response channel does not prove that
  // CLICK/TYPE did not execute. Navigation reconciliation owns that case.
...
function matchesCommandNavigationIntent(state, tab, context) {
  const intent = state?.command_navigation_intent;
  return Boolean(intent && intent.operation_id === Recovery.operationKey(state)
    && intent.tab_id === tab?.id && tab.windowId === state.current_window_id
    && directPublicAvitoUrl(intent.requested_url)
    && sameVisibleAvitoRoute(intent.requested_url, context?.url)
    && sameVisibleAvitoRoute(intent.requested_url, tab?.url)
    && !context?.login_popup && !["ip_block", "captcha"].includes(context?.page_kind));
}
async function preflightRouteContext(state, tab) {
  ...
  // Consume only an operation-owned, exact public navigation request. A route
  // changed manually, redirected elsewhere or belonging to an older operation
  // still goes through the original reconciliation/overlay safety boundary.
  if (matchesCommandNavigationIntent(state, tab, current)) {
    const latest = await assertCurrentOperation(state);
    const remembered = await rememberRouteContext({ ...latest, command_navigation_intent: null }, current, "assistant_requested_navigation");
    await log("AVITO_COMMAND_NAVIGATION_CONFIRMED", { search_id: state.search_id, tab_id: tab.id, current_url: current.url });
    return { changed: false, state: remembered, context: current, login_popup_events: [] };
  }
...
async function waitForSequentialCardReady(tabId, windowId, expectedHref, timeoutMs = SEQUENTIAL_CARD_READY_TIMEOUT_MS, ownerState = null) {
  const timeout = Math.max(1000, Math.min(Number(timeoutMs) || SEQUENTIAL_CARD_READY_TIMEOUT_MS, SEQUENTIAL_CARD_READY_TIMEOUT_MS));
  const deadline = Date.now() + timeout;
  return new Promise((resolve, reject) => {
    ...
    const assess = async (tab) => {
      if (settled) return;
      if (ownerState) {try {await assertCurrentOperation(ownerState);} catch(error) {finish(error);return;}}
      ...
      if (state.state === "ready") {
        if (ownerState?.avito_reload_expected === true) {const fresh=await probeAvitoPageReadiness(tab,windowId,ownerState);if(!fresh.ready) return;}
        finish(null, tab); return;
      }
      if (state.state === "loading_avito" && canonicalPublicListingUrl(state.url) === canonicalPublicListingUrl(expectedHref)) {
        const readiness = await probeAvitoPageReadiness(tab, windowId, ownerState);
...
function mergeSequentialDetails(previous, next) {
  // A fresh observation replaces a prior passport. Missing fields are unknown;
  // they must not silently inherit an earlier price/delivery/description.
  const fresh=next && typeof next==='object' ? next : {};
  return {...fresh,sections:Array.isArray(fresh.sections)?fresh.sections:[],seller_badges:Array.isArray(fresh.seller_badges)?fresh.seller_badges:[],delivery:Array.isArray(fresh.delivery)?fresh.delivery:[]};
}
...
function rawCollectedRecord(summary, details) {
  ...
  return {
    ...
    title: detail.title || summary?.title || "", price: detail.price || summary?.price || "",
    location: detail.location || "", seller: detail.seller || "", seller_rating: detail.seller_rating || "",
    seller_badges: Array.isArray(detail.seller_badges) ? detail.seller_badges : [],
    delivery: Array.isArray(detail.delivery) ? detail.delivery.join("; ") : (detail.delivery || ""),
    direct_delivery: detail.direct_delivery && typeof detail.direct_delivery === 'object' ? {...detail.direct_delivery} : null,
    availability: detail.availability || 'UNKNOWN',
    captured_at: detail.captured_at || null, source: detail.source || null,
    truncated_fields: Array.isArray(detail.truncated_fields) ? [...detail.truncated_fields] : [],
    sections: Array.isArray(detail.sections) ? detail.sections : [], description: detail.description || "",
...
function compactSequentialCheckpoint(checkpoint) {
  ...
  const collected = candidates.filter(summary => detailsByHref.has(sequentialDetailKey(summary))).map((summary) => {
    const row = detailsByHref.get(sequentialDetailKey(summary));
    return {...rawCollectedRecord(summary, row.details), queue_position:candidates.indexOf(summary)+1};
  });
  return {
    collector_kind: "assistant_explicit_public_url_queue",
    status: cp.status || "paused",
    candidate_count: candidates.length,
    details_collected: collected.length,
    cursor: Number(cp.cursor || 0),
    pending: candidates.slice(Number(cp.cursor || 0)).map((item,index) => ({href:item.href,queue_position:Number(cp.cursor || 0)+index+1,status:"NOT_READ"})),
    ...
    failed: failed.slice(0, EXPLICIT_QUEUE_MAX_ITEMS),
...
async function persistSequentialCheckpoint(state, checkpoint, status = null, phase = null) {
  const current = await assertCurrentOperation(state);
  const next = await saveState({ ...current, sequential_review: checkpoint, ...(status ? { status } : {}), ...(phase ? { phase } : {}) });
  return next;
}
...
function sequentialBlankUrl(value) {
  const url = String(value || "");
  return !url || /^(?:about:blank|chrome:\/\/newtab\/?)(?:#.*)?$/iu.test(url);
}
function sequentialTabExactTarget(tab, expectedHref) {
  const expected = canonicalPublicListingUrl(expectedHref);
  if (!tab || !expected) return false;
  return [tab.url, tab.pendingUrl].some((value) => Core.isAvitoUrl(String(value || "")) && canonicalPublicListingUrl(value) === expected);
}
function sequentialTabBootstrapOnly(tab) {
  if (!tab) return false;
  return sequentialBlankUrl(tab.url) && sequentialBlankUrl(tab.pendingUrl);
}
async function ownedSequentialChildren(parentTab) {
  const rows = await tabsQuery({ windowId: parentTab.windowId });
  return (Array.isArray(rows) ? rows : []).filter((tab) => tab?.id !== parentTab.id && tab.windowId === parentTab.windowId && Number(tab.openerTabId) === Number(parentTab.id));
}
async function adoptSequentialChild(parentTab, expectedHref, { allowBootstrap = false } = {}) {
  const owned = await ownedSequentialChildren(parentTab);
  const exact = owned.filter((tab) => sequentialTabExactTarget(tab, expectedHref));
  if (exact.length > 1) throw new Error(`SEQUENTIAL_TARGET_TAB_AMBIGUOUS:${exact.map((tab) => tab.id).join(",")}`);
  if (exact.length === 1) return { tab: exact[0], source: "exact_target" };
  if (!allowBootstrap) return { tab: null, source: "none" };
  const blank = owned.filter(sequentialTabBootstrapOnly);
  if (blank.length > 1) throw new Error(`SEQUENTIAL_BOOTSTRAP_TAB_AMBIGUOUS:${blank.map((tab) => tab.id).join(",")}`);
  if (blank.length === 1) return { tab: blank[0], source: "legacy_bootstrap" };
  return { tab: null, source: "none" };
}
async function configureSequentialTrafficLite(tabId) {
  try {
    await prepareTrafficLiteForNavigation(tabId);
    return true;
  } catch (error) {
    // Navigation ownership/readiness must not depend on an optional tab-scoped
    // traffic-saving rule. The global Avito initiator rule remains independent.
    await log("SEQUENTIAL_TRAFFIC_LITE_PREPARE_FAILED", { tab_id: tabId, error: Recovery.redactedError(error) });
    return false;
  }
}
async function ensureSequentialOwnedTargetTab(state, parentTab, checkpoint, summary, cursor) {
  const expectedHref = canonicalPublicListingUrl(summary?.href || "");
  if (!expectedHref) throw new Error("SEQUENTIAL_TARGET_URL_INVALID");
  let retained = checkpoint.in_flight || checkpoint.captcha || checkpoint.connection || null;
  let child = null;

  if (retained && Number(retained.cursor) === cursor && retained.child_tab_id) {
    child = await getTab(retained.child_tab_id);
    if (!child) throw new Error("SEQUENTIAL_RETAINED_TAB_CLOSED_BY_USER");
    if (child.windowId !== parentTab.windowId || Number(child.openerTabId) !== Number(parentTab.id)) throw new Error("SEQUENTIAL_RETAINED_TAB_OWNERSHIP_CHANGED");
    if (!sequentialTabExactTarget(child, expectedHref)) {
      // One-time migration/recovery for checkpoints produced by v1.0.30 and
      // earlier proxy-era builds that persisted an owned about:blank child.
      // New v1.0.31 runs never create this bootstrap tab.
      if (!sequentialTabBootstrapOnly(child)) throw new Error(`SEQUENTIAL_RETAINED_TAB_URL_MISMATCH:${child.id}`);
      checkpoint.in_flight = { ...retained, child_tab_id: child.id, cursor, href: expectedHref, stage: "LEGACY_BOOTSTRAP_TARGET_DISPATCH", navigation_requested_at: Date.now() };
      checkpoint.captcha = null; checkpoint.connection = null;
      await persistSequentialCheckpoint(state, checkpoint, "AVITO_TAB_ACTIVE", "EXECUTING_EXPLICIT_PUBLIC_QUEUE");
      await assertCurrentOperation(state);
      child = await tabsUpdate(child.id, { url: expectedHref, active: true });
      await log("SEQUENTIAL_LEGACY_BOOTSTRAP_MIGRATED", { search_id: state.search_id, cursor, child_tab_id: child.id, target_url: expectedHref });
    }
    await configureSequentialTrafficLite(child.id);
    return child;
  }

  // Persist intent BEFORE tabs.create. If the MV3 worker is terminated after
  // Chrome creates the tab but before its id is saved, the next wake can adopt
  // exactly one owned tab whose pending/committed route is the requested listing.
  const startedAt = retained?.started_at || Date.now();
  checkpoint.in_flight = { child_tab_id: null, cursor, href: expectedHref, started_at: startedAt, stage: "TARGET_TAB_CREATE_INTENT" };
  checkpoint.captcha = null; checkpoint.connection = null;
  await persistSequentialCheckpoint(state, checkpoint, "AVITO_TAB_ACTIVE", "EXECUTING_EXPLICIT_PUBLIC_QUEUE");

  const adopted = await adoptSequentialChild(parentTab, expectedHref, { allowBootstrap: true });
  if (adopted.tab) {
    child = adopted.tab;
    if (adopted.source === "legacy_bootstrap") {
      checkpoint.in_flight = { ...checkpoint.in_flight, child_tab_id: child.id, stage: "CRASH_BOOTSTRAP_TARGET_DISPATCH", navigation_requested_at: Date.now() };
      await persistSequentialCheckpoint(state, checkpoint);
      await assertCurrentOperation(state);
      child = await tabsUpdate(child.id, { url: expectedHref, active: true });
    }
    checkpoint.in_flight = { ...checkpoint.in_flight, child_tab_id: child.id, stage: "TARGET_TAB_ADOPTED" };
    await persistSequentialCheckpoint(state, checkpoint);
    await log("SEQUENTIAL_TARGET_TAB_ADOPTED", { search_id: state.search_id, cursor, child_tab_id: child.id, source: adopted.source, target_url: expectedHref });
    await configureSequentialTrafficLite(child.id);
    return child;
  }

  await assertCurrentOperation(state);
  // Pre-proxy lifecycle restored: create the owned child at the target listing
  // directly. Proxy/PAC and traffic-saving rules are transport concerns and do
  // not own the navigation handshake.
  child = await tabsCreate({ url: expectedHref, active: true, windowId: parentTab.windowId, openerTabId: parentTab.id });
  if (!child?.id) throw new Error("SEQUENTIAL_TARGET_TAB_CREATE_FAILED");
  checkpoint.in_flight = { ...checkpoint.in_flight, child_tab_id: child.id, stage: "TARGET_NAVIGATION_DISPATCHED", navigation_requested_at: Date.now() };
  await persistSequentialCheckpoint(state, checkpoint);
  await log("SEQUENTIAL_TARGET_TAB_CREATED", { search_id: state.search_id, cursor, child_tab_id: child.id, target_url: expectedHref, pending_url: child.pendingUrl || null, committed_url: child.url || null });
  await configureSequentialTrafficLite(child.id);
  return child;
}
async function readSequentialCardWithRecovery(state, child, expectedHref, checkpoint) {
  let readyState = null;
  while (true) {
    await assertCurrentOperation(state);
    let detail = null, error = null, ready = null;
    try {
      ready = await waitForSequentialCardReady(child.id, child.windowId, expectedHref, SEQUENTIAL_CARD_READY_TIMEOUT_MS, readyState || state);
      detail = await readSequentialPublicListing(ready.id, ready.windowId);
    } catch(e) { error = e; }
    await assertCurrentOperation(state);
    const evidence = await networkEvidenceForTab(child.id);
    const relevant = evidence && sameVisibleAvitoRoute(evidence.url, expectedHref) && evidence.at >= Math.max(Number(checkpoint.in_flight?.started_at || 0),Number(readyState?.avito_reload_requested_at || 0));
    const signal = Recovery.interruption({blocked_reason:detail?.blocked_reason,error:relevant?evidence.error:String(error?.message||''),status_code:relevant?evidence.status_code:null});
    if (signal === 'CAPTCHA') return {ready:ready || child,detail:{...detail,blocked_reason:'BLOCKED_LOGIN_OR_CAPTCHA'}};
    if (signal === 'RATE_LIMIT' || signal === 'IP_BLOCK' || signal === 'PROXY_TRANSPORT') {
      const current = await assertCurrentOperation(state);
      const target = await getTab(child.id);
      if (!target) return {ready:child,detail:{blocked_reason:'SEQUENTIAL_CARD_TAB_MISSING'}};
      const recovered = await recoverAvitoConnectionAndReload(current,target,readyState?.avito_reload_previous_time_origin || 0,'sequential_card',{
        kind:'sequential_card',parent_tab_id:checkpoint.parent_tab_id,cursor:checkpoint.cursor,target_url:expectedHref
      },signal);
      await assertCurrentOperation(state);
      if (recovered.status === 'RATE_LIMIT_BACKOFF') return {ready:target,detail:null,deferred_recovery:true};
      if (recovered.status !== 'SEQUENTIAL_RECOVERY_WAIT') return {ready:target,detail:{blocked_reason:recovered.blocked_reason || 'SEQUENTIAL_CONNECTION_RECOVERY_FAILED'}};
      readyState={...recovered,avito_tab_id:target.id,current_window_id:target.windowId,avito_requested_url:expectedHref};
      continue;
    }
    if (relevant && [404,410].includes(Number(evidence.status_code))) return {ready:ready || child,detail:{blocked_reason:`LISTING_UNAVAILABLE_HTTP_${evidence.status_code}`}};
    if (error) throw error;
    return {ready:ready || child,detail:detail || {blocked_reason:'SEQUENTIAL_LISTING_DETAILS_EMPTY'}};
  }
}
const sequentialGapTimers = new Map();
function scheduleSequentialGap(resumeAt) {
  // Timers are a foreground optimization only. Storage + alarm own the deadline.
  const key = 'gap';
  if (sequentialGapTimers.has(key)) clearTimeout(sequentialGapTimers.get(key));
  const delay = Math.max(1, Number(resumeAt) - Date.now());
  if (delay < 30000) sequentialGapTimers.set(key, setTimeout(() => {
    sequentialGapTimers.delete(key);
    recoverRuntimeOnWake().catch(error => log('SEQUENTIAL_GAP_WAKE_FAILED', {reason:Recovery.redactedError(error)}).catch(()=>{}));
  }, delay));
  scheduleRateLimitRecoveryAlarm(resumeAt);
}
async function finishSequentialPendingClose(state, parentTab, checkpoint) {
  const pending = checkpoint.pending_close;
  if (!pending) return;
  await assertCurrentOperation(state);
  const tab = await getTab(pending.child_tab_id);
  if (tab) {
    if (!await closeSequentialOwnedCard(tab.id, parentTab.id, parentTab.windowId)) throw new Error('SEQUENTIAL_OWNED_TAB_CLEANUP_UNCONFIRMED');
    checkpoint.cards_closed = Number(checkpoint.cards_closed || 0) + 1;
  } else {
    // The previous worker may have closed it before the close receipt was stored.
    // Keep count uncertainty explicit, but never reread a durable observation.
    checkpoint.close_receipt_unknown = Number(checkpoint.close_receipt_unknown || 0) + 1;
  }
  checkpoint.pending_close = null;
  await persistSequentialCheckpoint(state, checkpoint, 'AVITO_TAB_ACTIVE', 'SEQUENTIAL_CLOSE_RECONCILED');
}
async function runSequentialCardBatch(state, parentTab, checkpoint) {
  ...
  const batchKey = Recovery.operationKey(state);
  if (checkpoint.batch_operation_key !== batchKey || !Number.isInteger(checkpoint.batch_end_cursor)) {
    checkpoint.batch_operation_key = batchKey;
    checkpoint.batch_start_cursor = Number(checkpoint.cursor || 0);
    checkpoint.batch_end_cursor = Math.min(checkpoint.candidates.length, checkpoint.batch_start_cursor + batchTarget);
    await persistSequentialCheckpoint(state, checkpoint);
  }
  let attempted = 0;
  await finishSequentialPendingClose(state, parentTab, checkpoint);
  while (Number(checkpoint.cursor || 0) < checkpoint.batch_end_cursor) {
    await assertCurrentOperation(state);
    if (Date.now() < Number(checkpoint.resume_after || 0)) {
      checkpoint.status = 'waiting_between_cards';
      await persistSequentialCheckpoint(state, checkpoint, 'SEQUENTIAL_GAP_WAIT', 'SEQUENTIAL_CARD_GAP');
      scheduleSequentialGap(checkpoint.resume_after);
      return {checkpoint, attempted, blocked_reason:null, deferred_recovery:true};
    }
    checkpoint.resume_after = null;
    const cursor = Number(checkpoint.cursor || 0), summary = checkpoint.candidates[cursor];
    let child = null;
    try {
      child = await ensureSequentialOwnedTargetTab(state, parentTab, checkpoint, summary, cursor);
      await log('SEQUENTIAL_RETAINED_CARD_RECONCILED',{search_id:state.search_id,cursor,child_tab_id:child.id,source:checkpoint.captcha?'explicit_queue_captcha_manual_recheck':(checkpoint.in_flight?.stage || 'target_navigation')});
      armRuntimeWatchdog();
      const captured = await readSequentialCardWithRecovery(state,child,summary.href,checkpoint);
      if(captured?.deferred_recovery===true) {
        const deferredState=await getState();
        const deferredCheckpoint=deferredState.sequential_review && typeof deferredState.sequential_review==='object' ? deferredState.sequential_review : checkpoint;
        await log('SEQUENTIAL_RATE_LIMIT_BACKOFF_DEFERRED',{search_id:state.search_id,cursor,child_tab_id:child.id,cursor_advanced:false,resume_after:deferredState.connection_recovery?.resume_after||null});
        return {checkpoint:deferredCheckpoint,attempted,blocked_reason:null,deferred_recovery:true};
      }
      let ready=captured.ready, detail=captured.detail;
      if (detail?.blocked_reason === 'BLOCKED_LOGIN_OR_CAPTCHA') {
        ...
        if (interruption.captcha) {
          const media = await reloadCaptchaWithMediaIfNeeded(ready,parentTab.windowId,summary.href,interruption,'SEQUENTIAL_CAPTCHA_MEDIA_RELOAD_REQUESTED');
          if (media.reloaded) {ready=media.tab;detail=await readSequentialPublicListing(ready.id,parentTab.windowId);}
        }
      }
      await assertCurrentOperation(state);
      if (detail?.blocked_reason && !/^LISTING_UNAVAILABLE_HTTP_(?:404|410)$/.test(detail.blocked_reason)) {
        const captcha=detail.blocked_reason==='BLOCKED_LOGIN_OR_CAPTCHA';
        checkpoint.status=captcha?'paused_for_manual_captcha':'paused_for_connection';
        checkpoint.blocked_reason=captcha?'CAPTCHA_MANUAL_REQUIRED':detail.blocked_reason;
        checkpoint[captcha?'captcha':'connection']={...checkpoint.in_flight,title:summary.title||'',summary};
        await persistSequentialCheckpoint(state,checkpoint,'AVITO_TAB_ACTIVE','SEQUENTIAL_CHECKPOINT_PAUSED');
        await log(captcha?'SEQUENTIAL_CAPTCHA_MANUAL_PAUSE':'SEQUENTIAL_CONNECTION_CHECKPOINT_PAUSED',{search_id:state.search_id,cursor,child_tab_id:child.id,reason:checkpoint.blocked_reason,cursor_advanced:false});
        return {checkpoint,attempted,blocked_reason:checkpoint.blocked_reason};
      }
      if (detail?.blocked_reason) checkpoint.failed.push({title:summary.title||'',href:summary.href,reason:detail.blocked_reason});
      else {
        upsertSequentialDetail(checkpoint,summary,detail);
        const current=await assertCurrentOperation(state);
        if (current.connection_recovery?.tab_id===child.id) await log('AVITO_RECOVERY_TARGET_CONFIRMED',{search_id:state.search_id,attempt:current.connection_recovery.attempt,source:'sequential_public_detail'});
      }
      checkpoint.cursor=cursor+1;checkpoint.status='running';checkpoint.blocked_reason=null;
      checkpoint.pending_close={child_tab_id:child.id,cursor,href:summary.href};
      checkpoint.in_flight=null;checkpoint.captcha=null;checkpoint.connection=null;
      const cardGapMs = normalizeSequentialCardGapMs(checkpoint.card_gap_ms);
      checkpoint.resume_after=checkpoint.cursor<checkpoint.candidates.length ? Date.now()+cardGapMs : null;
      // Write the observation and cursor BEFORE closing the only document that
      // proves the read. A crash after close needs cleanup, not a second read.
      await persistSequentialCheckpoint(state,checkpoint,'AVITO_TAB_ACTIVE','SEQUENTIAL_RESULT_COMMITTED');
      attempted+=1;
      await finishSequentialPendingClose(state,parentTab,checkpoint);
    } catch(error) {
      const latest=await getState();
      if(!Recovery.sameOperation(state,latest) || latest.status==='CANCELLED_BY_USER') throw new Error('SEQUENTIAL_REVIEW_CANCELLED');
      const reason=Recovery.redactedError(error);
      // Do not replace the last durable checkpoint with an uncommitted local read
      // if storage failed. No cursor advance can be inferred from an exception.
      const durable=latest.sequential_review || checkpoint;
      durable.status='paused_for_connection';durable.blocked_reason=reason;
      durable.connection=durable.in_flight || (durable.pending_close ? null : {cursor:Number(durable.cursor||0),href:durable.candidates?.[Number(durable.cursor||0)]?.href||summary.href,child_tab_id:child?.id||null});
      await persistSequentialCheckpoint(state,durable,'AVITO_TAB_ACTIVE','SEQUENTIAL_CHECKPOINT_PAUSED');
      return {checkpoint:durable,attempted,blocked_reason:reason};
    }
  }
  ...
  await persistSequentialCheckpoint(state,checkpoint,'AVITO_TAB_ACTIVE','EXPLICIT_PUBLIC_QUEUE_BATCH_COMPLETE');
  return {checkpoint,attempted,blocked_reason:null};
}
async function runExplicitListingQueue(state, parentTab, plan) {
  if (!Array.isArray(plan.steps) || plan.steps.length !== 1) throw new Error("UI_PLAN_QUEUE_MUST_BE_ONLY_STEP");
  ...
  if (isStart) {
    ...
    if (requestedUrls.length !== action.selected_urls.length || requestedUrls.some(url=>!Core.isPublicListingUrl(url))) throw new Error("UI_PLAN_EXPLICIT_QUEUE_URL_NOT_ALLOWED");
    if (requestedUrls.length > EXPLICIT_QUEUE_MAX_ITEMS) throw new Error("UI_PLAN_EXPLICIT_QUEUE_TOO_LARGE");
    const candidates = requestedUrls.map((href) => ({ href, title: "" }));
    checkpoint = {
      version: 1,
  ...
  return {
    ...
    deferred_recovery: batch.deferred_recovery === true,
...
function continuationSequentialCheckpoint(checkpoint) {
  ...
  return ["running", "paused_after_test_batch", "paused_for_manual_captcha", "paused_for_connection"].includes(status) ? checkpoint : null;
}
const startAckFlight = Recovery.singleFlight();
const startAckTimers = new Map();
const START_ACK_BUDGET_MS = 120000;
const START_ACK_MAX_RECHECKS = 20;
function scheduleStartAcknowledgement(state, delay = 2000) {
  const key = Recovery.operationKey(state);
  const budget = state.start_ack_budget;
  if (!state.start_send_receipt || !budget || budget.attempt >= START_ACK_MAX_RECHECKS || Date.now() >= budget.deadline_at || state.status === 'CANCELLED_BY_USER') return false;
  if (startAckTimers.has(key)) clearTimeout(startAckTimers.get(key));
  startAckTimers.set(key,setTimeout(()=>{
    startAckTimers.delete(key);
    getState().then(current=>{
      if (Recovery.sameOperation(state,current) && current.status === 'START_DELIVERY_UNCERTAIN' && current.start_send_receipt && current.status !== 'CANCELLED_BY_USER') return reconcileUncertainStart(current);
    }).catch(error=>log('START_ACK_RECHECK_FAILED',{error:Recovery.redactedError(error)}));
  },delay));
  armRuntimeWatchdog();
  return true;
}
async function commitStartSendIntent(message,sender) {
  const state = await getState();
  if (state.status !== 'STARTING_BRIDGE_EXACT_CAPTURE' || state.search_id !== message.search_id || !Core.samePinnedChatContext(contextFromState(state),contextForTab(sender?.tab,message.identity))) return {ok:false,error:'START_SEND_INTENT_CONTEXT_MISMATCH'};
  const users = message.receipt?.baseline_user_turn_ids;
  const assistants = message.receipt?.baseline_assistant_turn_ids;
  const previous = message.receipt?.previous_anchor_turn_id || null;
  const visibleCount = message.receipt?.baseline_visible_user_count;
  const visibleExactCount = message.receipt?.baseline_visible_exact_user_count;
  const validIds = list => Array.isArray(list) && list.length <= 2000 && list.every(id=>typeof id === 'string' && id && id.length <= 200);
  const validCount = value => Number.isInteger(value) && value >= 0 && value <= 10000;
  if (!validIds(users) || !validIds(assistants) || !validCount(visibleCount) || !validCount(visibleExactCount) || visibleExactCount > visibleCount || (previous !== null && (typeof previous !== 'string' || previous.length > 200))) return {ok:false,error:'START_SEND_INTENT_INVALID_BASELINE'};
  await assertCurrentOperation(state);
  const saved = await saveState({...state,start_send_receipt:{schema:2,baseline_user_turn_ids:[...new Set(users)],baseline_assistant_turn_ids:[...new Set(assistants)],baseline_visible_user_count:visibleCount,baseline_visible_exact_user_count:visibleExactCount,previous_anchor_turn_id:previous,intent_at:Date.now()},start_send_attempted:null});
  if (!Recovery.sameOperation(state,saved) || saved.status === 'CANCELLED_BY_USER') return {ok:false,error:'START_SEND_INTENT_CANCELLED'};
  await log('START_SEND_INTENT_PERSISTED',{search_id:state.search_id,baseline_user_turn_count:users.length,baseline_assistant_turn_count:assistants.length,baseline_visible_user_count:visibleCount,baseline_visible_exact_user_count:visibleExactCount,previous_anchor_turn_id:previous});
  return {ok:true,data:{committed:true}};
}
async function reconcileUncertainStart(state) {
  const key = Recovery.operationKey(state);
  return startAckFlight(key,async()=>{
    let current = await getState();
    if (!Recovery.sameOperation(state,current) || current.status === 'CANCELLED_BY_USER') return {ok:false,state:current,error:'START_RECONCILE_STALE_OPERATION'};
    if (current.status !== 'START_DELIVERY_UNCERTAIN' || !current.start_send_receipt) return {ok:false,state:current,error:'NO_UNCERTAIN_START'};
    let pinned;
    try { pinned = await assertPinnedChatContext(current,'chatgpt_start_reconciliation',{activateTab:true}); }
    catch(error) {
      const priorBudget=current.start_ack_budget || {deadline_at:Date.now()+START_ACK_BUDGET_MS,attempt:0};
      const budget={deadline_at:priorBudget.deadline_at,attempt:Number(priorBudget.attempt||0)+1};
      const next=await saveState({...current,status:'START_DELIVERY_UNCERTAIN',phase:'CHATGPT_START_RECONCILIATION',start_send_attempted:true,start_ack_budget:budget,blocked_reason:Recovery.redactedError(error)});
      const scheduled=scheduleStartAcknowledgement(next,Math.min(5000,1000+budget.attempt*250));
      if(!scheduled) await log('START_ACK_BUDGET_EXHAUSTED',{search_id:next.search_id,attempt:budget.attempt,send_repeated:false,reason:'PINNED_CHAT_CONTEXT_UNAVAILABLE'});
      return {ok:false,state:next,error:next.blocked_reason,pending:scheduled};
    }
    let response;
    try {
      response = await sendChat(pinned.tab.id,{type:'AF_CAPTURE_RECONCILE_START',search_id:current.search_id,message_text:'Ищи',receipt:current.start_send_receipt,expected_identity:pinned.context});
    } catch(error) { response={ok:false,error:Recovery.redactedError(error)}; }
    current = await getState();
    if (!Recovery.sameOperation(state,current) || current.status === 'CANCELLED_BY_USER') return {ok:false,state:current,error:'START_RECONCILE_STALE_OPERATION'};
    if (response?.ok && response.state === 'confirmed' && response.anchor_turn_id && Core.sameConversationIdentity(pinned.context,response.identity)) {
      const waiting = await saveState({...current,status:'WAITING_FOR_ASSISTANT_WRITING_BLOCK',phase:'CHATGPT_CAPTURE',anchor_turn_id:response.anchor_turn_id,expected_assistant_turn_id:response.expected_assistant_turn_id || null,start_send_receipt:null,start_send_attempted:true,start_ack_budget:null,blocked_reason:null});
      const poll = await sendChat(pinned.tab.id,{type:'AF_CAPTURE_BEGIN_PROMPT_POLL',search_id:waiting.search_id,conversation_id:waiting.conversation_id,anchor_turn_id:waiting.anchor_turn_id,expected_assistant_turn_id:waiting.expected_assistant_turn_id || null,expected_identity:pinned.context});
      let final = await getState();
      if (!poll?.ok && Recovery.sameOperation(waiting,final) && final.status === 'WAITING_FOR_ASSISTANT_WRITING_BLOCK') final=await saveState({...final,status:'PROMPT_POLL_START_FAILED',blocked_reason:poll?.error || 'PROMPT_POLL_START_FAILED'});
      if (startAckTimers.has(key)) {clearTimeout(startAckTimers.get(key));startAckTimers.delete(key);}
      await log('START_RECONCILED_CONFIRMED',{search_id:final.search_id,anchor_turn_id:waiting.anchor_turn_id,expected_assistant_turn_id:waiting.expected_assistant_turn_id || null,receipt_mode:response.receipt_mode || null});
      return {ok:poll?.ok === true,state:final};
    }
    const priorBudget=current.start_ack_budget || {deadline_at:Date.now()+START_ACK_BUDGET_MS,attempt:0};
    const budget={deadline_at:priorBudget.deadline_at,attempt:Number(priorBudget.attempt||0)+1};
    const reason=response?.error || `START_RECEIPT_${String(response?.receipt_mode || response?.state || 'PENDING').toUpperCase()}`;
    const next=await saveState({...current,status:'START_DELIVERY_UNCERTAIN',phase:'CHATGPT_START_RECONCILIATION',start_send_attempted:true,start_ack_budget:budget,blocked_reason:reason});
    const scheduled=scheduleStartAcknowledgement(next,Math.min(5000,1000+budget.attempt*250));
    if(!scheduled) await log('START_ACK_BUDGET_EXHAUSTED',{search_id:next.search_id,attempt:budget.attempt,send_repeated:false});
    await log('START_RECONCILED_NOT_CONFIRMED',{search_id:next.search_id,response_state:response?.state || 'absent',receipt_mode:response?.receipt_mode || null,automatic_recheck_scheduled:scheduled});
    return {ok:true,state:next,pending:true};
  });
}
const reportFlight = Recovery.singleFlight();
const reportAckFlight = Recovery.singleFlight();
const reportDeliveryOwners = new Set();
const reportAckTimers = new Map();
const REPORT_ACK_BUDGET_MS = 120000;
const REPORT_ACK_MAX_RECHECKS = 12;
function scheduleReportAcknowledgement(state, delay = 3000) {
  const key = Recovery.operationKey(state);
  const budget = state.report_ack_budget;
  if (!state.report || !budget || budget.attempt >= REPORT_ACK_MAX_RECHECKS || Date.now() >= budget.deadline_at || state.status === 'CANCELLED_BY_USER') return false;
  if (reportAckTimers.has(key)) clearTimeout(reportAckTimers.get(key));
  reportAckTimers.set(key,setTimeout(()=>{
    reportAckTimers.delete(key);
    getState().then(current=>{
      if (Recovery.sameOperation(state,current) && current.report && current.status !== 'CANCELLED_BY_USER') return reconcileBlockedReport(current);
    }).catch(error=>log('REPORT_ACK_RECHECK_FAILED',{error:Recovery.redactedError(error)}));
  },delay));
  armRuntimeWatchdog();
  return true;
}
async function commitReportSendIntent(message,sender) {
  const state = await getState();
  if (!state.report || state.status !== 'REPORT_DELIVERY_IN_PROGRESS' || state.search_id !== message.search_id || state.report_delivery_id !== message.delivery_id || !Core.samePinnedChatContext(contextFromState(state),contextForTab(sender?.tab,message.identity))) return {ok:false,error:'REPORT_SEND_INTENT_CONTEXT_MISMATCH'};
  const ids = message.receipt?.baseline_user_turn_ids;
  if (!Array.isArray(ids) || ids.length > 2000 || ids.some(id=>typeof id !== 'string' || !id || id.length > 200)) return {ok:false,error:'REPORT_SEND_INTENT_INVALID_BASELINE'};
  await assertCurrentOperation(state);
  const saved = await saveState({...state,report_send_receipt:{schema:1,delivery_id:state.report_delivery_id,baseline_user_turn_ids:[...new Set(ids)],previous_anchor_turn_id:state.anchor_turn_id || null,intent_at:Date.now()},report_send_attempted:null});
  if (!Recovery.sameOperation(state,saved) || saved.status === 'CANCELLED_BY_USER') return {ok:false,error:'REPORT_SEND_INTENT_CANCELLED'};
  await log('REPORT_SEND_INTENT_PERSISTED',{search_id:state.search_id,baseline_user_turn_count:ids.length,delivery_id:state.report_delivery_id});
  return {ok:true,data:{committed:true}};
}
async function deliverReportToPinnedChat(state,report,reportKind) {
  const key = Recovery.operationKey(state);
  return reportFlight(key,async()=>{
    reportDeliveryOwners.add(key);
    try {return await deliverReportToPinnedChatUnlocked(state,report,reportKind);}
    finally {reportDeliveryOwners.delete(key);}
  });
}
async function deliverReportToPinnedChatUnlocked(state, report, reportKind) {
  const latest = await getState();
  if (!Recovery.sameOperation(state,latest) || latest.status === "CANCELLED_BY_USER") return latest;
  const deliveryId = latest.report_delivery_id || `${Recovery.operationKey(latest)}:${reportKind}:${Core.fingerprint(report)}`;
  const manualGateKind = reportKind === "avito_failure" && /CAPTCHA_MANUAL_REQUIRED/i.test(String(latest.blocked_reason || "")) ? "CAPTCHA" : null;
  const staged = await saveState({ ...latest, status: "REPORT_DELIVERY_IN_PROGRESS", phase: "CHATGPT_REPORT_DELIVERY", report, report_kind: reportKind, report_delivery_id: deliveryId, report_ack_budget:latest.report_ack_budget || {deadline_at:Date.now()+REPORT_ACK_BUDGET_MS,attempt:0}, manual_gate_kind: manualGateKind || latest.manual_gate_kind || null, blocked_reason: null });
  armRuntimeWatchdog();
  let pinned;
  try { pinned = await assertPinnedChatContext(staged, `chatgpt_return_after_${reportKind}`, { activateTab: true }); }
  catch(error) {
    const current=await getState();
    if (!Recovery.sameOperation(staged,current) || current.status==='CANCELLED_BY_USER' || error.state) return current;
    const blocked=await saveState({...current,status:'REPORT_DELIVERY_BLOCKED',phase:'CHATGPT_REPORT_DELIVERY',blocked_reason:Recovery.redactedError(error),report_delivery_dispatched:false});
    await log('REPORT_DELIVERY_CONTEXT_CHECK_FAILED',{search_id:current.search_id,reason:blocked.blocked_reason}); return blocked;
  }
  ...
  let response;
  try {
    await saveState({...await assertCurrentOperation(staged),report_delivery_dispatched:true,report_send_attempted:null});
    response = await sendChat(pinned.tab.id, { type: "AF_CAPTURE_SEND_REPORT", search_id: staged.search_id, delivery_id: deliveryId, report_text: report, previous_anchor_turn_id:staged.anchor_turn_id || null, expected_identity: pinned.context });
  ...
  if (!response?.ok || !Core.sameConversationIdentity(pinned.context, response.identity)) {
    ...
    const pause = response?.code === "PRIMARY_COMPOSER_OCCUPIED" || /PRIMARY_COMPOSER_OCCUPIED|composer.*другой|неотправленный текст/i.test(reason);
    const sendAttempted = typeof response?.send_attempted === "boolean" ? response.send_attempted : null;
    const current = await assertCurrentOperation(staged);
    const next = await saveState({ ...current, status: pause ? "PAUSED_USER_COMPOSER_OCCUPIED" : "REPORT_DELIVERY_BLOCKED", phase: "CHATGPT_REPORT_DELIVERY", blocked_reason: reason, report, report_kind: reportKind, report_delivery_id: deliveryId, report_send_attempted: sendAttempted, report_delivery_error_code: response?.code || "REPORT_DELIVERY_CONTEXT_UNCONFIRMED" });
    await log(pause ? "REPORT_DELIVERY_PAUSED_USER_COMPOSER_OCCUPIED" : "REPORT_DELIVERY_BLOCKED", { search_id: next.search_id, report_kind: reportKind, reason, code: response?.code || "", send_attempted: sendAttempted });
    if (!pause) scheduleReportAcknowledgement(next);
    return next;
  }
  await assertCurrentOperation(staged);
  ...
  const waiting = await saveState({
    ...staged,
    status: "WAITING_FOR_NEXT_ASSISTANT_FORM",
    phase: manualGateKind === "CAPTCHA" ? "CAPTCHA_MANUAL_WAIT" : "CHATGPT_CONTINUATION",
  ...
  if (!continuation?.ok) {
    const current = await assertCurrentOperation(waiting);
  ...
  await log("NEXT_FORM_POLL_STARTED", { search_id: waiting.search_id, report_kind: reportKind, anchor_turn_id: nextAnchorTurnId, chatgpt_tab_id: pinned.tab.id });
  // The content adapter may already have admitted the next command. Never
  // overwrite that newer state with the previous delivery's waiting snapshot.
  return getState();
}
...
function requestedRateLimitBackoffMs(record) {
  const testOverride = Number(globalThis.__AF_TEST_RATE_LIMIT_BACKOFF_MS);
  return Number.isFinite(testOverride) ? Math.max(0, testOverride) : Recovery.rateLimitBackoffMs(record.attempt);
}
const alarmWriteLane = Recovery.serialLane();
function scheduleOwnedAlarm(name, when) {
  if (!chrome.alarms?.create) return Promise.resolve(false);
  return alarmWriteLane(async()=>{
    const target=Math.max(Date.now()+1,Number(when)||Date.now()+1);
    const existing=chrome.alarms.get?await chrome.alarms.get(name):null;
    const old=Number(existing?.scheduledTime || existing?.when || 0);
    if(old>Date.now() && old<=target)return true;
    await chrome.alarms.create(name,{when:target});return true;
  }).catch(error=>{log('RUNTIME_ALARM_SCHEDULE_FAILED',{name,reason:Recovery.redactedError(error)}).catch(()=>{});return false;});
}
function scheduleRateLimitRecoveryAlarm(when) { return scheduleOwnedAlarm(RATE_LIMIT_RECOVERY_ALARM,when); }
async function avitoCooldownUntil() {
  const data=await storageGet('af_avito_rate_gate_v129');return Number(data.af_avito_rate_gate_v129?.until||0);
}
async function assertAvitoCooldown() {
  const until=await avitoCooldownUntil();
  if(until>Date.now()) throw new Error('AVITO_RATE_LIMIT_WAIT_REQUIRED:resume_after='+String(until));
}
async function persistRateLimitBackoff(current, record, tab, previousTimeOrigin, source, continuation) {
  const now=Date.now(), evidence=await networkEvidenceForTab(tab.id);
  const serverAt=evidence && sameVisibleAvitoRoute(evidence.url,tab.url) ? Number(evidence.retry_after_at||0) : 0;
  const resumeAt=Math.max(now+requestedRateLimitBackoffMs(record),serverAt,await avitoCooldownUntil());
  if(resumeAt<=now)return null;
  await storageSet({af_avito_rate_gate_v129:{until:resumeAt,source:serverAt>now?'RETRY_AFTER':'BOUNDED_BACKOFF',observed_at:now}});
  const exceedsBudget=resumeAt>=Number(record.deadline_at||0);
  const nextRecord={...record,phase:exceedsBudget?'RATE_LIMIT_WAIT_REQUIRED':'RATE_LIMIT_BACKOFF',resume_after:resumeAt,retry_after_at:serverAt||null,interruption_kind:'RATE_LIMIT',continuation:continuation||null,source,previous_time_origin:Number(previousTimeOrigin||0)};
  const next=await saveState({...current,status:exceedsBudget?'SEQUENTIAL_RECOVERY_BLOCKED':'RATE_LIMIT_BACKOFF',phase:'AVITO_RATE_LIMIT_BACKOFF',blocked_reason:exceedsBudget?'AVITO_RATE_LIMIT_WAIT_EXCEEDS_AUTOMATIC_BUDGET':null,connection_recovery:nextRecord,avito_ip_block_reload_count:record.attempt});
  if(exceedsBudget) {
    await log('AVITO_RATE_LIMIT_MANUAL_WAIT_REQUIRED',{search_id:next.search_id,tab_id:tab.id,resume_after:resumeAt,deadline_at:record.deadline_at});
    return continuation?next:blockAvitoPageReady(next,'AVITO_RATE_LIMIT_WAIT_EXCEEDS_AUTOMATIC_BUDGET',{resume_after:resumeAt});
  }
  await scheduleRateLimitRecoveryAlarm(resumeAt);armRuntimeWatchdog();
  await log('AVITO_RATE_LIMIT_BACKOFF_SCHEDULED',{search_id:next.search_id,tab_id:tab.id,attempt:record.attempt,delay_ms:resumeAt-now,resume_after:resumeAt,deadline_at:record.deadline_at,cursor:continuation?.cursor??null});
  return next;
}
async function performReservedConnectionRecoveryInsideLane(state, tab, previousTimeOrigin, source, continuation, interruptionKind, record) {
  let current=await assertCurrentOperation(state);
  if(current.status==='WAITING_FOR_NEXT_ASSISTANT_FORM') return current;
  const key=Recovery.operationKey(current);
  current=await saveState({...current,status:'RECOVERING_AVITO_CONNECTION',phase:'AVITO_CONNECTION_RECOVERY',connection_recovery:{...record,phase:'PREPARING',resume_after:null,continuation:continuation||record.continuation||null,source:source||record.source||null,previous_time_origin:Number(previousTimeOrigin||record.previous_time_origin||0),interruption_kind:interruptionKind||record.interruption_kind||'IP_BLOCK'},avito_network_authority:(current.avito_network_authority?.blocked===true?current.avito_network_authority:Recovery.beginBlockedEgressEpoch(current.avito_network_authority,interruptionKind||record.interruption_kind||'IP_BLOCK',record.attempt)),avito_ip_block_reload_count:record.attempt});
  const controller=new AbortController();
  recoveryControllers.set(key,controller);workerRecoveryOwner=key;armRuntimeWatchdog();
  const timer=setTimeout(()=>controller.abort(),Math.max(1,Number(record.deadline_at||0)-Date.now()));
  try {
    await log('AVITO_CONNECTION_RECOVERY_STARTED',{search_id:current.search_id,operation_id:key,tab_id:tab.id,attempt:record.attempt,source,interruption_kind:interruptionKind,resumed_reserved_attempt:true});
    await assertAvitoCooldown();
    // 429 is throttling, not evidence of a broken proxy. Respect the server and
    // retry the same selected route; no endpoint creation or IP rotation for 429.
    const prepared=await prepareRecoveryProxy(current,(await getState()).connection_recovery,controller.signal);
    ...
    if(!freshTab || freshTab.windowId!==current.current_window_id) throw new Error('AVITO_RECOVERY_TAB_UNAVAILABLE');
    const probe=await probeVisibleAvitoInterruption(freshTab);
    const targetObservation={target_ip_block:probe.ip_block===true,target_captcha:probe.captcha===true,target_rate_limit:probe.rate_limit===true,target_normal_avito:probe.normal_avito===true,target_url:probe.href||freshTab.url||null};
    if(probe.captcha) {
      await saveProxyRuntime({...await getProxyRuntime(),last_ip_block_recovery:{at:new Date().toISOString(),completed_at:new Date().toISOString(),attempt:record.attempt,interruption_kind:interruptionKind,...prepared,...targetObservation,ok:false,target_status:'CAPTCHA_MANUAL_REQUIRED',error:'CAPTCHA_MANUAL_REQUIRED'}});
      throw new Error('CAPTCHA_MANUAL_REQUIRED');
    }
    const origin=Number(probe.time_origin || previousTimeOrigin || record.previous_time_origin || 0);
    const at=Date.now();
    current=await saveState({...current,status:continuation?'SEQUENTIAL_RECOVERY_WAIT':'WAITING_FOR_AVITO_PAGE_READY',phase:'AVITO_RECOVERY_VERIFYING_TARGET',connection_recovery:{...current.connection_recovery,phase:'AWAITING_FRESH_DOCUMENT',resume_after:null,prepared},avito_network_authority:Recovery.applyAvitoEgressEvidence(current.avito_network_authority,prepared),avito_navigation_expected:false,avito_reload_expected:true,avito_reload_previous_time_origin:origin,avito_reload_requested_at:at,avito_ready_deadline_at:new Date(Math.min(at+AVITO_READY_TIMEOUT_MS,record.deadline_at)).toISOString()});
    await saveProxyRuntime({...await getProxyRuntime(),last_ip_block_recovery:{at:new Date().toISOString(),attempt:record.attempt,interruption_kind:interruptionKind,...prepared,...targetObservation,ok:null,target_status:'PENDING'}});
    await assertCurrentOperation(current);
    const target=directPublicAvitoUrl(continuation?.target_url || current.operation_page_url || freshTab.url);
    if(!target) throw new Error('AVITO_RECOVERY_TARGET_URL_MISSING');
    if(Core.isAvitoUrl(freshTab.url||'') && !new URL(freshTab.url).hash && sameVisibleAvitoRoute(freshTab.url,target)) await tabsReload(tab.id,AVITO_IP_BLOCK_RELOAD_PROPERTIES);
    else await tabsUpdate(tab.id,{url:target,active:true});
    await log('AVITO_RECOVERY_FRESH_DOCUMENT_REQUESTED',{search_id:current.search_id,attempt:record.attempt,source,interruption_kind:interruptionKind,probe_ip_changed:prepared.probe_ip_changed,avito_exit_ip_verified:false});
    if (!continuation) scheduleAvitoReadyDeadline(current);
    else { scheduleRateLimitRecoveryAlarm(Date.now()+1000); armRuntimeWatchdog(); }
    return current;
  } catch(error) {
    const latest=await getState();
    if(!Recovery.sameOperation(current,latest) || latest.status==='CANCELLED_BY_USER') return latest;
    const reason=controller.signal.aborted?'AVITO_RECOVERY_DEADLINE_EXCEEDED':Recovery.redactedError(error);
    const runtime=await getProxyRuntime();
    if(runtime.last_ip_block_recovery?.attempt!==record.attempt || runtime.last_ip_block_recovery?.target_status!=='CAPTCHA_MANUAL_REQUIRED') {
      const prepared=latest.connection_recovery?.prepared || current.connection_recovery?.prepared || {};
      await saveProxyRuntime({...runtime,last_ip_block_recovery:{...(runtime.last_ip_block_recovery||{}),at:new Date().toISOString(),completed_at:new Date().toISOString(),attempt:record.attempt,interruption_kind:interruptionKind,...prepared,ok:false,target_status:'RECOVERY_FAILED',error:reason}});
    }
    return continuation ? saveState({...latest,status:'SEQUENTIAL_RECOVERY_BLOCKED',blocked_reason:reason}) : blockAvitoPageReady(latest,reason,{source,attempt:record.attempt,interruption_kind:interruptionKind});
  } finally {clearTimeout(timer);recoveryControllers.delete(key);if(workerRecoveryOwner===key) workerRecoveryOwner=null;}
}
async function recoverAvitoConnectionAndReload(state, tab, previousTimeOrigin, source, continuation = null, interruptionKind = 'IP_BLOCK') {
  return recoveryFlight(`${Recovery.operationKey(state)}:${tab.id}`,()=>proxyControlLane(async()=>{
    let current=await assertCurrentOperation(state);
    if(current.status==='WAITING_FOR_NEXT_ASSISTANT_FORM') return current;
    if (continuation?.kind === 'sequential_card' && Recovery.replayPolicy(current.ui_action_plan) !== 'CHECKPOINT_ONLY') throw new Error('SEQUENTIAL_RECOVERY_POLICY_MISMATCH');
    if(current.command_mode==='AVITO_UI' && Recovery.replayPolicy(current.ui_action_plan)==='NEVER') return blockAvitoPageReady(current,'AVITO_INTERRUPTION_MUTATING_PLAN_NOT_REPLAYED',{source});
    if(current.command_mode==='DIAGNOSE_DOM' && current.diagnostic_request?.allow_navigation===false) return current;
    if(current.command_mode==='DIAGNOSE_DOM' && current.diagnostic_request?.action) return blockAvitoPageReady(current,'AVITO_INTERRUPTION_MUTATING_DIAGNOSTIC_NOT_REPLAYED',{source});
    let record;
    try {record=Recovery.reserveAttempt(Recovery.freshRecovery(current,tab.id,Date.now(),interruptionKind));}
    catch(error) {return continuation ? saveState({...current,status:'SEQUENTIAL_RECOVERY_BLOCKED',blocked_reason:error.message}) : blockAvitoPageReady(current,error.message,{source,interruption_kind:interruptionKind});}
    record={...record,continuation:continuation||null,source,previous_time_origin:Number(previousTimeOrigin||0),interruption_kind:interruptionKind};
    current=await saveState({...current,connection_recovery:record,avito_network_authority:Recovery.beginBlockedEgressEpoch(current.avito_network_authority,interruptionKind,record.attempt),avito_ip_block_reload_count:record.attempt});
    if(interruptionKind==='RATE_LIMIT') {
      const delayed=await persistRateLimitBackoff(current,record,tab,previousTimeOrigin,source,continuation);
      if(delayed) return delayed;
    }
    return performReservedConnectionRecoveryInsideLane(current,tab,previousTimeOrigin,source,continuation,interruptionKind,record);
  }));
}
async function resumePersistedConnectionRecovery(state) {
  const record=state?.connection_recovery;
  if(!record?.tab_id) return blockAvitoPageReady(state,'AVITO_RECOVERY_STATE_MISSING_TAB');
  if(Date.now()>=Number(record.deadline_at||0)) return blockAvitoPageReady(state,'AVITO_RECOVERY_INTERRUPTED_DEADLINE');
  if(state.status==='RATE_LIMIT_BACKOFF') {
    const resumeAt=Number(record.resume_after||0);
    if(resumeAt>Date.now()) {scheduleRateLimitRecoveryAlarm(resumeAt);armRuntimeWatchdog();return state;}
  }
  const tab=await getTab(record.tab_id);
  if(!tab) return record.continuation ? saveState({...state,status:'SEQUENTIAL_RECOVERY_BLOCKED',blocked_reason:'AVITO_RECOVERY_TAB_UNAVAILABLE'}) : blockAvitoPageReady(state,'AVITO_RECOVERY_TAB_UNAVAILABLE');
  const kind=record.interruption_kind||'IP_BLOCK';
  return recoveryFlight(`${Recovery.operationKey(state)}:${tab.id}`,()=>proxyControlLane(()=>performReservedConnectionRecoveryInsideLane(state,tab,Number(record.previous_time_origin||0),record.source||'runtime_recovery_resume',record.continuation||null,kind,record)));
}
async function recoverAvitoIpBlockAndReload(state, tab, previousTimeOrigin, source, continuation = null) {
  return recoverAvitoConnectionAndReload(state,tab,previousTimeOrigin,source,continuation,'IP_BLOCK');
}
async function completeAvitoTaskUnlocked(queuedState, readyTab, source, readinessHint = null) {
  const state = await getState();
  if (workerRecoveryOwner || !Recovery.sameOperation(state,queuedState) || state.status !== "WAITING_FOR_AVITO_PAGE_READY" || state.avito_tab_id !== readyTab.id) return;
  ...
  const latest = await getState(); if (!Recovery.sameOperation(state,latest) || latest.status !== "WAITING_FOR_AVITO_PAGE_READY") return;
  clearAvitoReadyDeadline(state.search_id);
  ...
  const network=await networkEvidenceForTab(verified.id);
  const networkRelevant=Boolean(network && network.at>=Math.max(Number(state.operation_started_at||Infinity),Number(state.avito_reload_requested_at||0)) && sameVisibleAvitoRoute(network.url,verified.url));
  const interruptionKind=Recovery.interruption({
    captcha:interruption.captcha,rate_limit:interruption.rate_limit,ip_block:interruption.ip_block,
    status_code:networkRelevant?network.status_code:null,error:networkRelevant?network.error:null
  });
  if (interruptionKind === 'RATE_LIMIT' && interruption.captcha !== true && !(state.command_mode === "DIAGNOSE_DOM" && state.diagnostic_request?.allow_navigation === false)) {
    clearAvitoReadyDeadline(state.search_id);
    await recoverAvitoConnectionAndReload(latest,verified,Number(interruption.time_origin||0),'post_navigation_rate_limit',null,'RATE_LIMIT');
    return;
  }
  if (interruptionKind === 'IP_BLOCK') interruption.ip_block=true;
  if (interruption.captcha === true && !(state.command_mode === "DIAGNOSE_DOM" && state.diagnostic_request?.allow_navigation === false)) {
    ...
    if (overrideAdded) {
      ...
      const reloading = await saveState({
        ...
        avito_reload_requested_at: Date.now(),
  ...
  if (interruption.ip_block === true && interruption.captcha !== true && !(state.command_mode === "DIAGNOSE_DOM" && state.diagnostic_request?.allow_navigation === false)) {
    clearAvitoReadyDeadline(state.search_id);
  ...
  if (working.command_mode === "DIAGNOSE_DOM") {
    ...
    if (snapshot.blocked_reason === 'AVITO_IP_BLOCK' && working.diagnostic_request?.allow_navigation !== false && !working.diagnostic_request?.action) {
      await storageSet({[INSPECTION_KEY]:{search_id:working.search_id,mode:'DIAGNOSE_DOM',avito:snapshot}});
      await recoverAvitoIpBlockAndReload(working,verified,0,'diagnostic_result'); return;
    }
  ...
  if (working.command_mode === "AVITO_UI") {
    ...
    try {
      ...
      if (hasExplicitQueueCollection) {
        ...
        if (result?.deferred_recovery === true || ['RATE_LIMIT_BACKOFF','RECOVERING_AVITO_CONNECTION','SEQUENTIAL_RECOVERY_WAIT'].includes(working.status)) {
          await log('EXPLICIT_PUBLIC_QUEUE_WAITING_FOR_CONNECTION_RECOVERY',{search_id:working.search_id,status:working.status,cursor:working.sequential_review?.cursor??null,resume_after:working.connection_recovery?.resume_after||null});
          return;
        }
    ...
    working=await assertCurrentOperation(working);
    await storageSet({[INSPECTION_KEY]:{search_id:working.search_id,operation_id:Recovery.operationKey(working),mode:"AVITO_UI",avito:result}});
    const uiPlanInterruption = Recovery.interruption(result || {});
    const safeCollectionReplay = ['IP_BLOCK','RATE_LIMIT','PROXY_TRANSPORT'].includes(uiPlanInterruption) && Recovery.replayPolicy(plan) === "READ_ONLY";
    if (safeCollectionReplay) {
      if (result.listings?.length) {
        const prior=Array.isArray(working.partial_collection_observations)?working.partial_collection_observations:[];
        working=await saveState({...working,partial_collection_observations:[...prior,{captured_at:new Date().toISOString(),url:result.url,listings:result.listings}].slice(-4)});
      }
      await log("AVITO_UI_PLAN_CONNECTION_RECOVERY_STARTED", {
        ...
        interruption_kind: uiPlanInterruption,
        source: "ui_plan_collection_connection_interruption"
      });
      const retryState = await saveState({
        ...working,
        status: "WAITING_FOR_AVITO_PAGE_READY",
        phase: "AVITO_UI_PLAN_CONNECTION_RECOVERY_HANDOFF",
        avito_tab_id: verified.id,
        blocked_reason: null
      });
      await recoverAvitoConnectionAndReload(retryState, verified, 0, "ui_plan_collection_connection_interruption", null, uiPlanInterruption);
      return;
    }
    if(!result.blocked_reason && ((result.listings||[]).length || result.listing_details || result.empty_results_confirmed)) {
      const rt=await getProxyRuntime();
      if(rt.last_ip_block_recovery && working.connection_recovery) {
        await saveProxyRuntime({...rt,last_ip_block_recovery:{...rt.last_ip_block_recovery,ok:true,target_status:'AVITO_PUBLIC_SURFACE_CONFIRMED',target_ip_block:false,target_captcha:false,target_rate_limit:false,target_normal_avito:true,completed_at:new Date().toISOString(),error:null}});
        await log('AVITO_RECOVERY_TARGET_CONFIRMED',{search_id:working.search_id,attempt:working.connection_recovery.attempt,source:'public_dom_collection'});
      }
    }
    ...
    if (working.partial_collection_observations?.length) result.prior_partial_observations=working.partial_collection_observations;
    const actionPlan = { search_id: working.search_id, mode: "AVITO_UI", captured_at: new Date().toISOString(), avito: result };
    const report = Core.formatUiActionPlanReport(actionPlan);
    await storageSet({ [INSPECTION_KEY]: actionPlan });
  ...
  if (snapshot.blocked_reason === 'AVITO_IP_BLOCK') {await recoverAvitoIpBlockAndReload(working,verified,0,'filter_result');return;}
...
async function queueAvitoTask(initialState, parsed) {
  await assertAvitoCooldown();
  await proxyControlLane(()=>restoreInterruptedProxyTransaction());
  initialState=await assertCurrentOperation(initialState);
  const targetRequest = buildAvitoTargetRequest(parsed);
  // Clear an older command's intent before any new visible navigation.
  initialState = await saveState({ ...initialState, command_navigation_intent: null });
  ...
  const state = await saveState({
    ...
    operation_page_url: avito.requested_url || targetRequest.requestedUrl || avito.tab.url,
...
function canResumePreClickReport(state) {
  if (state?.report_send_attempted === false) return true;
  if (state?.report_send_attempted === true) return false;
  // Narrow migration of the known v1.0.25 pre-click return, NOT a timeout.
  return state?.status === "REPORT_DELIVERY_BLOCKED" && state?.blocked_reason === "Текст в поле изменён после подготовки; пользовательский текст не отправлен и не перезаписан.";
}
async function reconcileBlockedReport(existingState = null, options = {}) {
  const snapshot = existingState || await getState();
  const key = Recovery.operationKey(snapshot);
  if (reportDeliveryOwners.has(key)) {armRuntimeWatchdog();return {ok:true,pending:true,state:await getState()};}
  return reportAckFlight(key,()=>reconcileBlockedReportUnlocked(snapshot,options));
}
async function reconcileBlockedReportUnlocked(existingState = null, { resume_preclick = false } = {}) {
  let state = await getState();
  if (existingState && !Recovery.sameOperation(existingState,state)) return {ok:true,ignored:true,state};
  if (state.status === 'CANCELLED_BY_USER') return {ok:true,ignored:true,state};
  if (!state?.report || !["REPORT_DELIVERY_BLOCKED", "REPORT_DELIVERY_IN_PROGRESS", "REPORT_READY_IN_COMPOSER", "REPORT_DELIVERY_UNCERTAIN", "PAUSED_USER_COMPOSER_OCCUPIED"].includes(state.status)) {
    return { ok: false, error: "NO_UNCERTAIN_REPORT", state };
  }
  const budget = state.report_ack_budget;
  const expired = budget && (budget.attempt >= REPORT_ACK_MAX_RECHECKS || Date.now() >= budget.deadline_at);
  if (expired && !resume_preclick) return {ok:true,exhausted:true,state};
  state = await saveState({...state,report_ack_budget:!budget || (expired && resume_preclick) ? {deadline_at:Date.now()+REPORT_ACK_BUDGET_MS,attempt:1} : {...budget,attempt:budget.attempt+1}});
  let pinned;
  ...
  try {
    response = await sendChat(pinned.tab.id, {
      ...
      receipt:state.report_send_receipt || null,
      expected_identity: pinned.context
    });
  ...
  if (!response?.ok || !Core.sameConversationIdentity(pinned.context, response.identity)) {
    ...
    scheduleReportAcknowledgement(next);
    return { ok: false, error: next.blocked_reason, state: next };
  }
  await assertCurrentOperation(state);
  const resumablePreClick = canResumePreClickReport(state);
  if (response.state === "confirmed" && response.anchor_turn_id) {
    await storageRemove([INSPECTION_KEY]);
    const waiting = await saveState({ ...state, status: "WAITING_FOR_NEXT_ASSISTANT_FORM", phase: "CHATGPT_CONTINUATION", anchor_turn_id: response.anchor_turn_id, report: null, report_kind: null, report_delivery_id: null, report_send_receipt:null,report_ack_budget:null,report_send_attempted:null,report_delivery_dispatched:false,report_delivery_error_code:null,partial_collection_observations:[], sequential_review: continuationSequentialCheckpoint(state.sequential_review), blocked_reason: null });
    ...
    let final = await getState();
    if (!poll?.ok && Recovery.sameOperation(waiting,final) && final.status === 'WAITING_FOR_NEXT_ASSISTANT_FORM') final = await saveState({...final,blocked_reason:poll?.error || 'NEXT_FORM_POLL_START_FAILED'});
    const timerKey=Recovery.operationKey(state);if(reportAckTimers.has(timerKey)){clearTimeout(reportAckTimers.get(timerKey));reportAckTimers.delete(timerKey);}
  ...
  if (resume_preclick && resumablePreClick && ["staged", "empty"].includes(response.state)) {
    await assertCurrentOperation(state);
    await log("REPORT_PRECLICK_RESUME_REQUESTED", { search_id: state.search_id, response_state: response.state, send_attempted: false });
    const resumed = await deliverReportToPinnedChat(state, state.report, state.report_kind || "ui_action_plan");
    return { ok: resumed.status === "WAITING_FOR_NEXT_ASSISTANT_FORM", state: resumed };
  }
  const nextStatus = response.state === "staged" ? "REPORT_READY_IN_COMPOSER" : response.state === "occupied" ? "PAUSED_USER_COMPOSER_OCCUPIED" : "REPORT_DELIVERY_UNCERTAIN";
  const next = await saveState({ ...state, status: nextStatus, phase: "CHATGPT_REPORT_RECONCILIATION", report_send_attempted: resumablePreClick ? false : state.report_send_attempted ?? null, blocked_reason: nextStatus === "REPORT_READY_IN_COMPOSER" ? "REPORT_STILL_IN_PRIMARY_COMPOSER" : response.state === "occupied" ? "REPORT_EDITOR_CONTENT_MISMATCH__SAVED_REPORT_PRESERVED" : "REPORT_ABSENT_AFTER_UNCERTAIN_SEND" });
  // Keep bounded READ-ONLY acknowledgement active for both uncertain sends and
  // proven pre-click staging failures. If the operator manually sends the staged
  // report, the next pass will confirm the exact user turn and release prompt
  // polling. Automatic reconciliation never clicks Send.
  const scheduled = response.state !== 'occupied' && scheduleReportAcknowledgement(next);
  if (!scheduled && response.state !== 'occupied') {
    await log('REPORT_ACK_BUDGET_EXHAUSTED',{search_id:next.search_id,attempt:next.report_ack_budget?.attempt || 0,send_repeated:false});
  }
  await log("REPORT_RECONCILED_NOT_CONFIRMED", { search_id: next.search_id, response_state: response.state || "absent",automatic_recheck_scheduled:scheduled === true });
  return { ok: true, state: next };
}
async function begin() {
  const prior = await getState();
  if (prior.status === 'START_DELIVERY_UNCERTAIN' && prior.start_send_receipt) return reconcileUncertainStart(prior);
  if (proxyUiOwner || workerRecoveryOwner || ['STARTING_BRIDGE_EXACT_CAPTURE','FORM_TEXT_CAPTURED_UNVALIDATED','COMMAND_CAPTURED','AVITO_TAB_ACTIVE','WAITING_FOR_AVITO_PAGE_READY','RECOVERING_AVITO_CONNECTION','RATE_LIMIT_BACKOFF','SEQUENTIAL_GAP_WAIT','SEQUENTIAL_RECOVERY_WAIT','WAITING_FOR_UI_CLICK_RECONCILIATION'].includes(prior.status)) return {ok:false,error:'CURRENT_OPERATION_IN_PROGRESS',state:prior};
  if (prior.report && prior.status !== "CANCELLED_BY_USER") {
    if (["REPORT_DELIVERY_BLOCKED","REPORT_DELIVERY_IN_PROGRESS","REPORT_READY_IN_COMPOSER","REPORT_DELIVERY_UNCERTAIN","PAUSED_USER_COMPOSER_OCCUPIED"].includes(prior.status)) return reconcileBlockedReport(prior);
    return {ok:false,error:"PENDING_REPORT_MUST_BE_DELIVERED",state:prior};
  }
  let active;
  ...
  let startResult;
  try {
    startResult = await sendChat(pinned.tab.id, { type: "AF_CAPTURE_START_AND_ANCHOR", search_id: searchId, message_text: "Ищи", wait_for_conversation_identity: true, expected_identity: pinned.context });
  } catch (error) {
    const reason = String(error?.message || error);
    const current = await getState();
    if (Recovery.sameOperation(state,current) && current.start_send_receipt) {
      state = await saveState({ ...current, status: "START_DELIVERY_UNCERTAIN", phase: "CHATGPT_START_RECONCILIATION", start_ack_budget: current.start_ack_budget || {deadline_at:Date.now()+START_ACK_BUDGET_MS,attempt:0}, blocked_reason: reason });
      scheduleStartAcknowledgement(state);
      await log("START_DELIVERY_UNCERTAIN", { search_id: searchId, reason, code: "START_CAPTURE_TRANSPORT_AFTER_INTENT", send_repeated:false });
      return { ok: false, state, error: reason };
    }
    state = await saveState({ ...state, status: "START_CAPTURE_TRANSPORT_FAILED", phase: "CHATGPT_START", blocked_reason: reason }); await log("START_CAPTURE_TRANSPORT_FAILED", { search_id: searchId, reason }); return { ok: false, state, error: reason };
  }
  const currentAfterStart = await getState();
  if (!Recovery.sameOperation(state,currentAfterStart) || currentAfterStart.status === 'CANCELLED_BY_USER') return {ok:false,state:currentAfterStart,error:'START_OPERATION_CANCELLED'};
  if (!startResult?.ok || !startResult.anchor_turn_id || !Core.sameConversationIdentity(pinned.context, startResult.identity)) {
    const reason = startResult?.error || "START_EXACT_ANCHOR_OR_CONTEXT_UNCONFIRMED";
    if (startResult?.send_attempted === true || currentAfterStart.start_send_receipt) {
      state = await saveState({ ...currentAfterStart, status: "START_DELIVERY_UNCERTAIN", phase: "CHATGPT_START_RECONCILIATION", start_send_attempted:startResult?.send_attempted === true ? true : currentAfterStart.start_send_attempted, start_ack_budget: currentAfterStart.start_ack_budget || {deadline_at:Date.now()+START_ACK_BUDGET_MS,attempt:0}, blocked_reason: reason });
      scheduleStartAcknowledgement(state);
      await log("START_DELIVERY_UNCERTAIN", { search_id: searchId, reason, code: startResult?.code || null, send_attempted:startResult?.send_attempted === true, send_repeated:false });
      return { ok: false, state, error: reason };
    }
    state = await saveState({ ...currentAfterStart, status: "START_CAPTURE_FAILED", phase: "CHATGPT_START", blocked_reason: reason }); await log("START_CAPTURE_FAILED", { search_id: searchId, reason, code: startResult?.code || null }); return { ok: false, state, error: reason };
  }
  state = await saveState({ ...currentAfterStart, status: "WAITING_FOR_ASSISTANT_WRITING_BLOCK", phase: "CHATGPT_CAPTURE", anchor_turn_id: startResult.anchor_turn_id, expected_assistant_turn_id:startResult.expected_assistant_turn_id || null, start_send_receipt:null, start_send_attempted:startResult.send_attempted === true, start_ack_budget:null, blocked_reason: null }); await log("BRIDGE_EXACT_START_SENT_AND_ANCHORED", { search_id: searchId, anchor_turn_id: startResult.anchor_turn_id, expected_assistant_turn_id:startResult.expected_assistant_turn_id || null, start_receipt_mode:startResult.start_receipt_mode || null, conversation_id: state.conversation_id, content_script_version: startResult.content_script_version || null });
  const poll = await sendChat(pinned.tab.id, { type: "AF_CAPTURE_BEGIN_PROMPT_POLL", search_id: searchId, conversation_id: state.conversation_id, anchor_turn_id: startResult.anchor_turn_id, expected_assistant_turn_id:startResult.expected_assistant_turn_id || null, expected_identity: pinned.context });
  ...
  await log("BRIDGE_EXACT_PROMPT_POLL_STARTED", { search_id: searchId, anchor_turn_id: startResult.anchor_turn_id, expected_assistant_turn_id:startResult.expected_assistant_turn_id || null, conversation_id: state.conversation_id }); return { ok: true, state };
}
function validationErrorReport(state, parsed, text) {
  ...
  return [
    ...
    "Расширение вернуло ошибку в этот же чат. Следующее сообщение пользователя может быть обычным текстом. Исполняемый ответ ассистента Finder принимает только как Writing Block; обычный Markdown/code block командой не является."
  ].join("\n");
}
...
async function returnAvitoRuntimeFailureToPinnedChat(state, reason) {
  const latest=await getState();
  if(!Recovery.sameOperation(state,latest) || latest.status==='CANCELLED_BY_USER') return latest;
  const op=Recovery.operationKey(state);
  if(latest.terminal_report_operation_id===op) return latest;
  state=await saveState({...latest,terminal_report_operation_id:op});
...
function assistantFormErrorReport(state, candidate) {
  return [
    "Avito Finder: команда не запущена — исполняемый Writing Block отсутствует.",
    "",
    `Задача: ${state.search_id || "?"}`,
    `Assistant turn: ${candidate.assistant_turn_id || "не определён"}`,
    "Причина: ASSISTANT_WRITING_BLOCK_REQUIRED",
    "",
    "Assistant-ответ уже завершён, но Finder не нашёл Writing Block.",
    "Обычный текст и Markdown/code block намеренно не исполняются как команда.",
    "На Avito ничего не открывалось и не изменялось.",
    "Следующее сообщение пользователя может быть обычным текстом; следующий исполняемый ответ ассистента должен быть Writing Block."
  ].join("\n");
}
async function handlePromptFormError(message, sender) {
  let state = await getState();
  ...
  const actual = { tab_id: sender?.tab?.id ?? null, window_id: sender?.tab?.windowId ?? null, origin: candidate.origin, chat_path: candidate.chat_path, conversation_id: candidate.conversation_id };
  if (!Core.samePinnedChatContext(expected, actual)) return { ok: true, data: { ignored: true, reason: "FORM_ERROR_CONTEXT_MISMATCH" } };
  if (candidate.anchor_turn_id !== state.anchor_turn_id) return { ok: true, data: { ignored: true, reason: "STALE_ANCHOR_CANDIDATE" } };
  if (!["WAITING_FOR_ASSISTANT_WRITING_BLOCK", "WAITING_FOR_NEXT_ASSISTANT_FORM", "PAUSED_CHAT_CONTEXT_CHANGED"].includes(state.status)) {
    return { ok: true, data: { ignored: true, reason: "COMMAND_ALREADY_IN_FLIGHT" } };
  }
  if (candidate.rejection_reason !== "ASSISTANT_WRITING_BLOCK_REQUIRED" || candidate.assistant_finality_confirmed !== true) {
    return { ok: true, data: { ignored: true, reason: "FORM_ERROR_NOT_CONFIRMED" } };
  }
  const report = assistantFormErrorReport(state, candidate);
  const staged = await saveState({
    ...state,
    status: "VALIDATION_ERROR_REPORT_READY",
    phase: "CHATGPT_CAPTURE_FORMAT",
    assistant_turn_id: candidate.assistant_turn_id || null,
    blocked_reason: "ASSISTANT_WRITING_BLOCK_REQUIRED",
    report,
    report_kind: "validation_error"
  });
  await log("ASSISTANT_WRITING_BLOCK_REQUIRED", {
    search_id: staged.search_id,
    anchor_turn_id: staged.anchor_turn_id,
    assistant_turn_id: candidate.assistant_turn_id || null,
    avito_mutated: false
  });
  const delivered = await deliverReportToPinnedChat(staged, report, "validation_error");
  return { ok: true, data: { accepted: true, disposition: "continuation_started", continuation_anchor_turn_id: delivered.anchor_turn_id || null, reason: "ASSISTANT_WRITING_BLOCK_REQUIRED" } };
}
...
async function handleFullText(message, sender) {
  ...
  const promptWaitingStatuses = ['WAITING_FOR_ASSISTANT_WRITING_BLOCK','WAITING_FOR_NEXT_ASSISTANT_FORM','PAUSED_CHAT_CONTEXT_CHANGED'];
  if (!promptWaitingStatuses.includes(state.status)) {
    // v1.0.34 migration/recovery: a truncated same-turn Writing Block could be
    // prematurely validated by older builds and leave an UNSENT validation
    // report staged in the primary composer. If the exact same assistant turn
    // later exposes a strictly larger payload, discard only that Finder-owned
    // unsent validation stage and allow the complete payload to supersede it.
    const candidateBytes = Number(candidate.payload_bytes || 0);
    const priorLengthMatch = String(state.last_consumed_form_key || '').match(/\|(\d+):[0-9a-f]+$/iu);
    const priorCapturedLength = Number(priorLengthMatch?.[1] || 0);
    const sameAssistantTurn = Boolean(state.assistant_turn_id && candidate.assistant_turn_id && state.assistant_turn_id === candidate.assistant_turn_id);
    const unsentValidationStage = Boolean(
      state.report_kind === 'validation_error' &&
      ['VALIDATION_ERROR_REPORT_READY','REPORT_DELIVERY_BLOCKED','REPORT_DELIVERY_IN_PROGRESS','REPORT_READY_IN_COMPOSER'].includes(state.status) &&
      state.report_send_attempted !== true &&
      state.report &&
      sameAssistantTurn &&
      candidate.payload_extracted === true &&
      candidateBytes > priorCapturedLength &&
      priorCapturedLength > 0
    );
    if (unsentValidationStage) {
      const discarded = await sendChat(state.chatgpt_tab_id, {
        type: 'AF_CAPTURE_DISCARD_OWNED_REPORT_STAGE',
        search_id: state.search_id,
        delivery_id: state.report_delivery_id || '',
        report_text: state.report,
        report_fingerprint: Core.fingerprint(state.report),
        expected_identity: contextFromState(state)
      });
      if (discarded?.ok && discarded?.cleared === true) {
        await log('TRUNCATED_VALIDATION_SUPERSEDED_BY_LARGER_SAME_TURN_PAYLOAD', {
          search_id: state.search_id,
          assistant_turn_id: candidate.assistant_turn_id || null,
          prior_captured_length: priorCapturedLength,
          replacement_payload_bytes: candidateBytes,
          prior_report_send_attempted: state.report_send_attempted === true
        });
        state = await saveState({
          ...state,
          status: 'WAITING_FOR_NEXT_ASSISTANT_FORM',
          phase: 'WRITING_BLOCK_PAYLOAD_SUPERSEDE',
          report: null,
          report_kind: null,
          report_delivery_id: null,
          report_send_attempted: null,
  ...
  if (candidate.manual_interruption === true && state.manual_gate_kind === "CAPTCHA") {
    const nextAnchorTurnId = String(candidate.next_user_turn_id || "").trim();
    if (!nextAnchorTurnId || nextAnchorTurnId.length > 200) {
      return { ok: true, data: { ignored: true, reason: "CAPTCHA_MANUAL_TURN_ID_INVALID" } };
    }
    state = await saveState({
      ...state,
      status: "WAITING_FOR_NEXT_ASSISTANT_FORM",
      phase: "CAPTCHA_MANUAL_WAIT",
      anchor_turn_id: nextAnchorTurnId,
      assistant_turn_id: null,
      blocked_reason: null
    });
    await log("CAPTCHA_MANUAL_USER_TURN_REANCHORED", {
      search_id: state.search_id,
      anchor_turn_id: nextAnchorTurnId,
      prior_anchor_turn_id: candidate.anchor_turn_id || null,
      user_text_read: false
    });
    return { ok: true, data: { reanchored: true, continuation_anchor_turn_id: nextAnchorTurnId, reason: "CAPTCHA_MANUAL_GATE_REANCHORED" } };
  }
  ...
  // Pure parsing happens before durable command ownership. A service-worker
  // termination can therefore never strand the run in an unvalidated state or
  // consume a form key before deterministic local validation completes.
  const parsed = Core.parseCommandForm(text);
  await log("LOCAL_COMMAND_VALIDATION_COMPLETED", { search_id: state.search_id, valid: parsed.valid, errors: parsed.errors, mode: parsed.mode, mode_source: parsed.mode_source, candidate_count: parsed.candidate_count, candidate_count_source: parsed.candidate_count_source, text_fingerprint: Core.fingerprint(text), form_key: formKey });
  const capturedState = {
    ...state,
    operation_id: `${state.search_id}:${formKey}`,
    allow_new_operation: true,
    connection_recovery: null,
    terminal_report_operation_id: null,
    assistant_turn_id: candidate.assistant_turn_id || null,
    last_consumed_form_key: formKey,
    captured_payload_bytes: claimedPayloadBytes,
    captured_text_fingerprint: Core.fingerprint(text),
    blocked_reason: null
  };
  if (!parsed.valid) {
    const delivered = await returnValidationErrorToPinnedChat(capturedState, parsed, text);
  ...
  if (parsed.mode === "SEARCH") {
    const report = "Avito Finder: режим SEARCH является устаревшим и не выполняется. Для поиска используйте AVITO_UI, публичную Страницу и явные Шаги. Avito не изменялся.";
  ...
  state = await saveState({ ...state, status: "COMMAND_CAPTURED", phase: "COMMAND_CAPTURED", command_mode: parsed.mode, diagnostic_request: parsed.diagnostic || null, ui_action_plan: parsed.ui_plan || null, manual_gate_kind: null, blocked_reason: null });
  queueAvitoTask(state, parsed).catch(async (error) => {
    const reason = String(error?.message || error);
    const latest = await getState();
    if (!Recovery.sameOperation(state,latest) || latest.status === "CANCELLED_BY_USER") return;
...
async function resumeSequentialReviewFromPopup(state) {
  ...
  const staged = await saveState({ ...state, status: "COMMAND_CAPTURED", phase: "SEQUENTIAL_REVIEW_RESUME_REQUESTED", operation_id: `${state.search_id}:resume:${Date.now()}`, allow_new_operation: true, connection_recovery: null, terminal_report_operation_id: null, command_mode: "AVITO_UI", ui_action_plan: plan, avito_tab_id: parent.id, blocked_reason: null });
...
async function continueReport() {
  const state = await getState();
  if (state.status === "START_DELIVERY_UNCERTAIN" && state.start_send_receipt) return reconcileUncertainStart(state);
  if (["REPORT_DELIVERY_BLOCKED", "REPORT_DELIVERY_IN_PROGRESS", "REPORT_READY_IN_COMPOSER", "REPORT_DELIVERY_UNCERTAIN", "PAUSED_USER_COMPOSER_OCCUPIED"].includes(state.status) && state.report) return reconcileBlockedReport(state, { resume_preclick: true });
...
async function stop() {
  for (const timer of sequentialGapTimers.values()) clearTimeout(timer); sequentialGapTimers.clear();
  if (chrome.alarms?.clear) { chrome.alarms.clear(RATE_LIMIT_RECOVERY_ALARM); chrome.alarms.clear("avito-finder-runtime-v125"); }
  for(const timer of reportAckTimers.values())clearTimeout(timer);
  reportAckTimers.clear();
  for(const timer of startAckTimers.values())clearTimeout(timer);
  startAckTimers.clear();
  const old=await getState();clearAvitoReadyDeadline(old.search_id);clearUiClickReconcileDeadline(old.search_id);
  const state=await saveState({...old,status:'CANCELLED_BY_USER',phase:'STOPPED',blocked_reason:'CANCELLED_BY_USER',user_started:false});
  for(const controller of recoveryControllers.values()) controller.abort();
  for (const tabId of new Set([old.avito_tab_id,old.sequential_review?.in_flight?.child_tab_id].filter(Number.isInteger))) tabsSend(tabId,{type:'AF_CANCEL_AVITO_OPERATION'}).catch(()=>{});
  if(old.chatgpt_tab_id && old.conversation_id) {
    try {await sendChat(old.chatgpt_tab_id,{type:'AF_CAPTURE_STOP',expected_identity:contextFromState(old)});} catch(error) {await log('CAPTURE_STOP_CONTEXT_BLOCKED',{reason:Recovery.redactedError(error)});}
  }
  await log('STOPPED_BY_USER',{search_id:state.search_id});return state;
}
async function view() { const raw = await storageGet([STATE_KEY, LOG_KEY, INSPECTION_KEY]); return { ok: true, state: Core.makeState(raw[STATE_KEY] || {}), logs: raw[LOG_KEY] || [], inspection: raw[INSPECTION_KEY] || null, proxy: await proxyView() }; }
async function recoverContinuousPromptPoll(message, sender) {
  let state = await getState();
  const validationSupersedeWait = Boolean(
    state.report_kind === 'validation_error' &&
    ['VALIDATION_ERROR_REPORT_READY','REPORT_DELIVERY_BLOCKED','REPORT_DELIVERY_IN_PROGRESS','REPORT_READY_IN_COMPOSER'].includes(state.status) &&
    state.report_send_attempted !== true &&
    state.assistant_turn_id &&
    state.last_consumed_form_key
  );
  if (!["WAITING_FOR_NEXT_ASSISTANT_FORM", "WAITING_FOR_ASSISTANT_WRITING_BLOCK", "PAUSED_CHAT_CONTEXT_CHANGED"].includes(state.status) && !validationSupersedeWait || !state.search_id || !state.anchor_turn_id) {
  ...
  if (!["WAITING_FOR_NEXT_ASSISTANT_FORM","WAITING_FOR_ASSISTANT_WRITING_BLOCK"].includes(state.status) && !validationSupersedeWait) return { ok: true, data: { resume: false, recovered: true, state } };
  await log(validationSupersedeWait ? "VALIDATION_SUPERSEDE_POLL_RECOVERY_GRANTED" : "NEXT_FORM_POLL_RECOVERY_GRANTED", { search_id: state.search_id, anchor_turn_id: state.anchor_turn_id, chatgpt_tab_id: state.chatgpt_tab_id });
  return { ok: true, data: { resume: true, search_id: state.search_id, conversation_id: state.conversation_id, anchor_turn_id: state.anchor_turn_id, expected_assistant_turn_id: state.expected_assistant_turn_id || null, expected_identity: expected } };
}
...
if (globalThis.__AF_TEST_EXPORTS) {
  globalThis.__AF_TEST_EXPORTS.recovery = {
    begin,continueReport,reconcileBlockedReport,canResumePreClickReport,commitReportSendIntent,scheduleReportAcknowledgement,commitStartSendIntent,reconcileUncertainStart,scheduleStartAcknowledgement,
    getState,saveState,getProxyRuntime,saveProxyRuntime,getProxySecret,saveProxySecret,savePersistedProxyProfiles,
    log,patchProxyDiagnostics,applyProxyProfileInternal,prepareRecoveryProxy,restoreInterruptedProxyTransaction,assertAvitoNetworkRequestAuthorized,
    maybeCreateRecoveryEndpoint,reserveRecoveryEndpoint,recoverAvitoIpBlockAndReload,recoverRuntimeOnWake,
    completeAvitoTask,blockAvitoPageReady,stop,deliverReportToPinnedChat,returnAvitoRuntimeFailureToPinnedChat,recoverAvitoConnectionAndReload,resumePersistedConnectionRecovery,persistRateLimitBackoff,
    networkEvidenceForTab,recordNetworkEvidence,handleFullText,handlePromptFormError,queueAvitoTask,runSequentialCardBatch,runExplicitListingQueue,
    continuationSequentialCheckpoint,readSequentialCardWithRecovery,ensureSequentialOwnedTargetTab,adoptSequentialChild,ownedSequentialChildren,proxyUiRequest,sendAvito,
    saveManualTrafficProfile,deleteProxyProfileLocally,setProxyDirect,syncProxyMarket,migrateLegacyProxyProfiles,
    owner:()=>workerRecoveryOwner,
    clearTimers:()=>{for(const t of sequentialGapTimers.values())clearTimeout(t);sequentialGapTimers.clear();for(const t of reportAckTimers.values())clearTimeout(t);reportAckTimers.clear();for(const t of startAckTimers.values())clearTimeout(t);startAckTimers.clear();for(const t of avitoReadyTimers.values())clearTimeout(t);for(const t of uiClickReconcileTimers.values())clearTimeout(t);}
  };
  ...
  globalThis.__AF_TEST_EXPORTS.optionalLogin = { preflightRouteContext, matchesCommandNavigationIntent, isAssistantDirectedOptionalLoginDialogInspection, isAssistantDirectedOptionalLoginGenericClick, optionalLoginPreflightDisposition };
  globalThis.__AF_TEST_EXPORTS.readiness = { normalizedVisibleAvitoUrl, sameVisibleAvitoRoute, avitoNavigationCommitObserved, avitoReadyProbeAccepted, probeAvitoPageReadiness, sequentialCardUrlState, waitForSequentialCardReady, sequentialBlankUrl, sequentialTabExactTarget, sequentialTabBootstrapOnly, fixedAvitoReadyDomProbe, AVITO_IP_BLOCK_RELOAD_PROPERTIES };
}
...
if (chrome.webRequest?.onAuthRequired?.addListener) {
  chrome.webRequest.onAuthRequired.addListener((details, rawCallback) => {
    let replied = false; const callback = value => { if (replied) return; replied = true; rawCallback(value); };
    if (!details?.isProxy) { callback({}); return; }
    Promise.all([getProxySecret(), getProxyRuntime()]).then(async ([secret, runtime]) => {
      const profile = (secret.profiles || []).find((item) => String(item.id) === String(secret.pending_auth_profile_id || secret.active_profile_id || runtime.selected_profile_id || ""));
      const activeProxyMode = (runtime.mode === "proxy" || Boolean(secret.pending_auth_profile_id)) && Boolean(profile);
      ...
      await patchProxyDiagnostics({ last_auth_at: authAt, last_auth_profile_id: profile.id, last_auth_host: ProxyCore.text(details.challenger?.host, 255), last_auth_port: ProxyCore.int(details.challenger?.port, null), last_auth_attempt: attempts, last_auth_failure_at: null, last_auth_failure: null });
...
const networkWriteLane=Recovery.serialLane();
async function recordNetworkEvidence(details, kind) { return networkWriteLane(async()=>{
  if(!Number.isInteger(details.tabId) || details.tabId<0)return false;
  const raw=await sessionGet('af_network_v125'), map=raw.af_network_v125 || {};
  const key=String(details.tabId), prior=map[key] || {}, now=Date.now();
  if(kind!=='started' && prior.request_id && prior.request_id!==String(details.requestId)) return false;
  const header=Array.isArray(details.responseHeaders)?details.responseHeaders.find(h=>String(h.name||'').toLowerCase()==='retry-after'):null;
  const retryAfter=header?Recovery.retryAfterTime(header.value,now):(kind==='started'?null:prior.retry_after_at||null);
  map[key]={...(kind==='started'?{}:prior),kind,request_id:String(details.requestId||''),url:details.url||prior.url||'',at:now,started_at:kind==='started'?now:prior.started_at||0,status_code:details.statusCode||null,error:details.error||null,from_cache:details.fromCache===true,retry_after_at:retryAfter};
  // Only the derived deadline is kept. Never persist response headers/cookies.
  if(Number(details.statusCode)===429 && retryAfter>now && Core.isAvitoUrl(details.url)) {
    await storageSet({af_avito_rate_gate_v129:{until:Math.max(await avitoCooldownUntil(),retryAfter),source:'RETRY_AFTER',observed_at:now}});
  }
  const keys=Object.keys(map).sort((a,b)=>Number(map[b].at||0)-Number(map[a].at||0));
  for(const stale of keys.slice(100))delete map[stale];
  await sessionSet({af_network_v125:map}); return true;
}); }
async function networkEvidenceForTab(tabId) { const raw=await sessionGet('af_network_v125'); return raw.af_network_v125?.[String(tabId)] || null; }
if(chrome.webRequest?.onBeforeRequest?.addListener) chrome.webRequest.onBeforeRequest.addListener(d=>{if(d.type==='main_frame') recordNetworkEvidence(d,'started').catch(()=>{});},{urls:['https://avito.ru/*','https://*.avito.ru/*']});

if(chrome.webRequest?.onHeadersReceived?.addListener) chrome.webRequest.onHeadersReceived.addListener(d=>{if(d.type==='main_frame') recordNetworkEvidence(d,'headers').catch(()=>{});},{urls:['https://avito.ru/*','https://*.avito.ru/*']},['responseHeaders']);

if (chrome.webRequest?.onCompleted?.addListener) {
  chrome.webRequest.onCompleted.addListener((details) => {
    if (details?.type !== "main_frame") return;
    recordNetworkEvidence(details,"completed").then(()=>reconcilePendingAvitoReadiness()).catch(()=>{});
    const at = new Date().toISOString();
...
if (chrome.webRequest?.onErrorOccurred?.addListener) {
  chrome.webRequest.onErrorOccurred.addListener((details) => {
    if (details?.type !== "main_frame") return;
    const evidenceWrite = recordNetworkEvidence(details,"error");
    const at = new Date().toISOString();
    ...
    if (!PROXY_TRANSPORT_RECOVERABLE_ERRORS.has(errorText)) {evidenceWrite.catch(()=>{});return;}
    (async () => {
      if (!await evidenceWrite) return;
      const state = await getState();
      const evidence = await networkEvidenceForTab(details.tabId);
      if (evidence?.request_id !== String(details.requestId)) return;
      if (workerRecoveryOwner || !state.search_id || state.status !== "WAITING_FOR_AVITO_PAGE_READY" || state.avito_tab_id !== details.tabId) return;
      const runtime = await getProxyRuntime();
      if (runtime.mode !== "proxy") return;
      await withAvitoCompletionLock(state.search_id, async () => {
        const latest = await getState();
        if (latest.status !== "WAITING_FOR_AVITO_PAGE_READY" || latest.avito_tab_id !== details.tabId) return;
        ...
        await recoverAvitoConnectionAndReload(latest, tab, 0, "proxy_transport_error", null, "PROXY_TRANSPORT");
      });
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => { (async () => { try {
  ...
  if(['AF_START','AF_STOP','AF_CONTINUE_REPORT','AF_GET_VIEW'].includes(type) && !trustedExtensionUiSender(sender)) {sendResponse({ok:false,error:'EXTENSION_UI_SENDER_REQUIRED'});return;}
  if(type.startsWith('AF_CAPTURE_') && type!=='AF_CAPTURE_PING' && !trustedContentSender(sender,'chat')) {sendResponse({ok:false,error:'PINNED_CHAT_SENDER_REQUIRED'});return;}
  if((type.startsWith('AF_DEBUGGER_') || type==='AF_UI_CLICK_DISPATCHED') && !trustedContentSender(sender,'avito')) {sendResponse({ok:false,error:'PUBLIC_AVITO_SENDER_REQUIRED'});return;}
  if (String(message?.type || '').startsWith('AF_PROXY_')) {
    if(!trustedExtensionUiSender(sender)) {sendResponse({ok:false,error:'PROXY_UI_SENDER_REQUIRED'}); return;}
    const reads = {AF_PROXY_DIAGNOSTIC:()=>proxyDiagnosticView(),AF_PROXY_PROFILE_DETAILS:()=>getProxyProfileDetails(message.profile_id),AF_PROXY_KEY_EXPORT:()=>exportProxyKeyBundle(message)};
    sendResponse(await (reads[message.type] ? reads[message.type]() : proxyUiRequest(message))); return;
  }
  switch (message?.type) {
    case "AF_START": sendResponse(await startFlight("start",()=>begin())); return;
    ...
    case "AF_CAPTURE_REPORT_SEND_INTENT": sendResponse(await commitReportSendIntent(message,sender)); return;
    case "AF_CAPTURE_START_SEND_INTENT": sendResponse(await commitStartSendIntent(message,sender)); return;
    ...
    case "AF_CAPTURE_FULL_TEXT": sendResponse(await captureLane(()=>handleFullText(message, sender))); return;
    case "AF_CAPTURE_FORM_ERROR": sendResponse(await captureLane(()=>handlePromptFormError(message, sender))); return;
...
if(chrome.alarms?.onAlarm?.addListener) chrome.alarms.onAlarm.addListener(alarm=>{if(alarm.name==='avito-finder-runtime-v125' || alarm.name===RATE_LIMIT_RECOVERY_ALARM) recoverRuntimeOnWake().catch(e=>log('RUNTIME_RECOVERY_WAKE_FAILED',{error:Recovery.redactedError(e),alarm:alarm.name}));});
if(chrome.runtime.onStartup?.addListener) chrome.runtime.onStartup.addListener(()=>recoverRuntimeOnWake().catch(()=>{}));
if(chrome.alarms?.create) recoverRuntimeOnWake().catch(e=>log('RUNTIME_RECOVERY_WAKE_FAILED',{error:Recovery.redactedError(e)}));
