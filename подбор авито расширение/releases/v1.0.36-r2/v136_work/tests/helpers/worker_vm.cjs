...
function loadWorker(options={}) {
  ...
  const timerFn=(fn,ms,...args)=>{if(disposed)return null;const t=setTimeout(()=>{timers.delete(t);fn(...args);},ms);timers.add(t);return t;};
  ...
  return {h,R,P,C,ctx,chrome,local,session,calls,sent,tabs,alarms,identity,profile,seed,effective:()=>effective,dispose(){disposed=true;for(const t of timers)clearTimeout(t);h.clearTimers();}};
}
