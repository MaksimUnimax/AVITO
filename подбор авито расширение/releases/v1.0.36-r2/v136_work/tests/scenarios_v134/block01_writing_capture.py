...
async def main():
 ...
 async with async_playwright() as pw:
  ...
  async def s6():
   await add(VALID,ordinary=True)
   # Current ChatGPT also exposes the generic assistant-turn Copy after finality.
   await page.evaluate('()=>{const s=document.querySelector("section[data-turn=assistant]");const b=document.createElement("button");b.dataset.testid="copy-turn-action-button";b.ariaLabel="Копировать ответ";b.textContent="Копировать ответ";s.append(b)}')
   await start();await page.wait_for_timeout(650)
   commands=await calls();errors=await page.evaluate('sentMessages.filter(m=>m.type==="AF_CAPTURE_FORM_ERROR")')
   await page.wait_for_timeout(450);errors2=await page.evaluate('sentMessages.filter(m=>m.type==="AF_CAPTURE_FORM_ERROR")')
   return {'command_count':len(commands),'format_error_count':len(errors),'later_format_error_count':len(errors2),'pass':len(commands)==0 and len(errors)==1 and len(errors2)==1}
  await run('B01-S06','ordinary markdown code block','code-block-viewer + local Copy + completed assistant-turn Copy, no writing block','zero commands; one terminal same-chat form error',s6)
