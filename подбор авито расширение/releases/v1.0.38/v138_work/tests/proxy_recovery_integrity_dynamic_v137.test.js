...
function popupRequest(w,message){
  ...
  return new Promise((resolve,reject)=>{
    let timer=setTimeout(()=>reject(new Error('POPUP_REQUEST_TEST_TIMEOUT')),3000);
...
// Dynamic counterpart to the live diagnostic defect: PAC mode alone is not enough.
...
// Same-profile PAC churn is an implementation detail; it must not erase evidence
// that this active profile already supplied credentials in the profile epoch.
...
// Live run reported attempt 3 while popup showed attempt 2. Current attempt must be
// committed before the manual CAPTCHA terminal exit.
...
// Loss of session API key after browser/update must not silently look like endpoint
// creation was attempted or possible.
test('dynamic attempt 3 without session API key records explicit endpoint-create skip', async t=>{
  ...
  const returned=await w.h.maybeCreateRecoveryEndpoint(s,record,w.profile,controller.signal);
  assert.equal(returned.id,w.profile.id);
  const after=await w.h.getState();
  assert.deepEqual(JSON.parse(JSON.stringify(after.connection_recovery.create)),{state:'SKIPPED',reason:'API_KEY_UNAVAILABLE',attempt:3,at:after.connection_recovery.create.at});
...
// Probe failures must remain visible after the bounded prepare returns; logs alone
// are not durable enough for popup/user diagnosis.
...
// Manual CAPTCHA status must survive successful report delivery immediately; wake
// re-probe is fallback/migration, not the primary state transport.
