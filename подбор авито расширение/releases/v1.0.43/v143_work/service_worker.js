...
async function performReservedConnectionRecoveryInsideLane(state, tab, previousTimeOrigin, source, continuation, interruptionKind, record) {
  ...
  try {
    ...
    const prepared=interruptionKind==='RATE_LIMIT' ? {strategy:'COOLDOWN_SAME_ROUTE',probe_ip_changed:false,avito_exit_ip_verified:false} : await prepareRecoveryProxy(current,(await getState()).connection_recovery,controller.signal);
...
async function recoverAvitoConnectionAndReload(state, tab, previousTimeOrigin, source, continuation = null, interruptionKind = 'IP_BLOCK') {
  return recoveryFlight(`${Recovery.operationKey(state)}:${tab.id}`,()=>proxyControlLane(async()=>{
    ...
    current=await saveState({...current,connection_recovery:record,avito_ip_block_reload_count:record.attempt});
...
if (globalThis.__AF_TEST_EXPORTS) {
  globalThis.__AF_TEST_EXPORTS.recovery = {
    ...
    log,patchProxyDiagnostics,applyProxyProfileInternal,prepareRecoveryProxy,restoreInterruptedProxyTransaction,
